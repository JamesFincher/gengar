...
selectable via ``gengar tools`` → Image Generation; the active model is
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(
                ...
                f"  • Pick a different model via `gengar tools` → Image Generation."
            ) from exc
        raise
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping gengar with a FAL key configured
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `gengar plugins list` to see "
