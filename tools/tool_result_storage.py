...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/gengar-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/gengar-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/gengar-results"
HEREDOC_MARKER = "GENGAR_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/gengar-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"GENGAR_PERSIST_{uuid.uuid4().hex[:8]}"
