...
Uses the Vercel Python SDK to run commands in cloud sandboxes through Gengar'
shared ``BaseEnvironment`` shell contract. When persistence is enabled, the
backend stores task-scoped snapshot metadata under ``GENGAR_HOME`` and restores
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _configure_attached_sandbox(self, *, requested_cwd: str) -> None:
        ...
        if self._remote_home == "/":
            container_base = "/.gengar"
        else:
            container_base = f"{self._remote_home.rstrip('/')}/.gengar"
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        remote_hermes = (
            "/.gengar"
            if self._remote_home == "/"
            else f"{self._remote_home.rstrip('/')}/.gengar"
        )
        archive_member = remote_hermes.lstrip("/")
        remote_tar = f"/tmp/.gengar_sync.{os.getpid()}.tar"
        sandbox = self._sandbox
        if sandbox is None:
            raise RuntimeError("Vercel sandbox is not attached")
