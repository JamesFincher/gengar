...
_FORCE_SYNC_ENV = "GENGAR_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.gengar") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.gengar to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``GENGAR_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, hermes_home: Path | None = None) -> None:
        ...
        Downloads the remote ``.gengar/`` directory as a tar archive,
        ...
        # against an uninitialized remote .gengar/ directory.
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="gengar-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.gengar/skills/a.md`` →
        ``~/.gengar/skills/a.md``, a new remote file at
        ``/root/.gengar/skills/b.md`` maps to ``~/.gengar/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
