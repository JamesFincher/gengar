...
def _load_hermes_env_vars() -> dict[str, str]:
    """Load ~/.gengar/.env values without failing Docker command execution."""
    try:
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``GENGAR_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("GENGAR_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using GENGAR_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image entrypoint drops from root to the 'gengar'
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.gengar/sandboxes/). Non-persistent
        ...
        container_name = f"gengar-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Gengar secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _GENGAR_PROVIDER_ENV_BLOCKLIST)
