...
This module provides the glue:
    - ``GengarTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Gengar"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses GENGAR_HOME so each profile gets its own OAuth tokens.
    Layout: ``GENGAR_HOME/mcp-tokens/``
    """
    try:
        from hermes_constants import get_hermes_home
        base = Path(get_hermes_home())
    except ImportError:
        base = Path(os.environ.get("GENGAR_HOME", str(Path.home() / ".gengar")))
...
# ---------------------------------------------------------------------------
# GengarTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class GengarTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        GENGAR_HOME/mcp-tokens/<server_name>.json         -- tokens
        GENGAR_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Gengar records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Gengar.</p></body></html>"
            ) if code else (
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = GengarTokenStorage(server_name)
    storage.remove()
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Gengar")
    scope = cfg.get("scope")
