...
Architecture:
    ~/.gengar/checkpoints/{sha256(abs_dir)[:16]}/   — shadow git repo
        HEAD, refs/, objects/                        — standard git internals
        GENGAR_WORKDIR                               — original dir path
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("GENGAR_CHECKPOINT_TIMEOUT", "30"))))
...
def _git_env(shadow_repo: Path, working_dir: str) -> dict:
    ...
    The shadow repo is internal Gengar infrastructure — it must NOT inherit
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.email", "gengar@local"], shadow_repo, working_dir)
    _run_git(["config", "user.name", "Gengar Checkpoint"], shadow_repo, working_dir)
    ...
    (shadow_repo / "GENGAR_WORKDIR").write_text(
...
#   * orphan:  the ``GENGAR_WORKDIR`` path no longer exists on disk
...
def _read_workdir_marker(shadow_repo: Path) -> Optional[str]:
    """Read ``GENGAR_WORKDIR`` from a shadow repo, or None if missing/unreadable."""
    try:
        return (shadow_repo / "GENGAR_WORKDIR").read_text(encoding="utf-8").strip()
...
def prune_checkpoints(
    ...
    * ``delete_orphans=True`` and its ``GENGAR_WORKDIR`` path no longer
...
def maybe_auto_prune_checkpoints(
    ...
    bounded regardless of how many times gengar is invoked per day.
