...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.gengar/config.yaml``. Gengar
...
Configuration is loaded from ~/.gengar/config.yaml under the 'tts:' key.
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.gengar/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.gengar/config.yaml.
...
# custom voice-cloning scripts, etc.) into Gengar without any Python code
...
#           output_format: wav
#
# Gengar writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__GENGAR_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # output_format when Gengar actually needs a non-default format/override.
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Gengar caches Piper voice models.

    Resolves to ``~/.gengar/cache/piper-voices/`` under the active
    GENGAR_HOME so voice downloads follow profile boundaries.
    """
    from hermes_constants import get_hermes_dir
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.gengar/config.yaml (tts: section).
    ...
    platform = get_session_env("GENGAR_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'gengar[mistral]'"
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run gengar setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'gengar setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'gengar tools' and select Piper under TTS, or install manually: "
