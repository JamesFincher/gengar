...
discovers their tools, and registers them into the gengar tool registry
so the agent can call them like any built-in tool.

Configuration is read from ~/.gengar/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.gengar/logs/mcp-stderr.log), tagged with the
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            hermes_home = os.path.expanduser(
                os.getenv(
                    "GENGAR_HOME", os.path.join(os.path.expanduser("~"), ".gengar")
                )
            )
            candidates = [
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `gengar mcp login {server_name}` (or delete the tokens "
            f"file under ~/.gengar/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Gengar config file.
    ...
    ``os.environ`` (which includes ``~/.gengar/.env`` loaded at startup).
    """
    try:
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Gengar's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Gengar registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``gengar tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Gengar registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple gengar
