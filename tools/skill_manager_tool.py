...
~/.gengar/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `gengar config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.gengar/skills/ (single source of truth)
...
def _pinned_guard(name: str) -> Optional[str]:
    ...
    ``gengar curator unpin <name>`` (or edit it directly by hand).  This
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`gengar curator unpin {name}` if they want the change."
            )
    except Exception:
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.gengar/skills/) first, then any
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "pointing the user to `gengar curator unpin <name>`. Don't try to route "
