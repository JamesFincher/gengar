...
def _is_hermes_provider_credential(name: str) -> bool:
    """True if ``name`` is a Gengar-managed provider credential (API key,
    token, or similar) per ``_GENGAR_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Gengar API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    in the blocklist and remain legitimately registerable — skills that
    wrap third-party APIs still work.
    """
    try:
        from tools.environments.local import _GENGAR_PROVIDER_ENV_BLOCKLIST
    except Exception:
        return False
    return name in _GENGAR_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Gengar-managed provider credentials (from
    ``_GENGAR_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Gengar-managed
    ...
    Non-Gengar third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if _is_hermes_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Gengar provider "
                "credential %r (blocked by _GENGAR_PROVIDER_ENV_BLOCKLIST). "
