...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.gengar",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``GENGAR_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside GENGAR_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    hermes_home = _resolve_hermes_home()

    # Reject absolute paths — they bypass the GENGAR_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to GENGAR_HOME)",
            relative_path,
        )
        return False
    ...
    # that traversal like ``../. ssh/id_rsa`` cannot escape GENGAR_HOME.
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.gengar/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="gengar-skills-safe-"))
