"""Persistent slash-command worker — one GengarCLI per TUI session.
...
def _run(cli: GengarCLI, command: str) -> str:
...
def main():
    ...
    os.environ["GENGAR_SESSION_KEY"] = args.session_key
    os.environ["GENGAR_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = GengarCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
