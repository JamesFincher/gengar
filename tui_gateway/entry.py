...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `GENGAR_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("GENGAR_TUI_SIDECAR_URL")
...
# tested; override via ``GENGAR_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("GENGAR_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``GENGAR_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
