"""
Gengar MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Gengar-specific extra)

Usage:
    gengar mcp serve
    gengar mcp serve --verbose
...
logger = logging.getLogger("gengar.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using GENGAR_HOME."""
    try:
        from hermes_constants import get_hermes_home
        return get_hermes_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("GENGAR_HOME", Path.home() / ".gengar")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        ...
        directory_file = Path(
            os.environ.get("GENGAR_HOME", Path.home() / ".gengar")
...
class EventBridge:
    ...
    This is the Gengar equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            ...
            db_file = Path(os.environ.get("GENGAR_HOME", Path.home() / ".gengar")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Gengar MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "gengar",
        instructions=(
            "Gengar messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Gengar MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
