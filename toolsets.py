...
_GENGAR_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (GENGAR_KANBAN_TASK env set), otherwise
...
TOOLSETS = {
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (GENGAR_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full Gengar toolsets (CLI + messaging platforms)
    #
    ...
    "gengar-cli": {
        ...
        "tools": _GENGAR_CORE_TOOLS,
        "includes": []
    },

    "gengar-cron": {
        # Mirrors gengar-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `gengar tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant, rl) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as gengar-cli; gated by `gengar tools`",
        "tools": _GENGAR_CORE_TOOLS,
        "includes": []
    },
    ...
    "gengar-email": {
        "description": "Email bot toolset - interact with Gengar via email (IMAP/SMTP)",
        "tools": _GENGAR_CORE_TOOLS,
        "includes": []
    },
    ...
    "gengar-sms": {
        "description": "SMS bot toolset - interact with Gengar via SMS (Twilio)",
        "tools": _GENGAR_CORE_TOOLS,
        "includes": []
    },
    ...
    "gengar-gateway": {
        ...
        "tools": [],
        "includes": ["gengar-telegram", "gengar-discord", "gengar-whatsapp", "gengar-slack", "gengar-signal", "gengar-bluebubbles", "gengar-homeassistant", "gengar-email", "gengar-sms", "gengar-mattermost", "gengar-matrix", "gengar-dingtalk", "gengar-feishu", "gengar-wecom", "gengar-wecom-callback", "gengar-weixin", "gengar-qqbot", "gengar-webhook", "gengar-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (gengar-<name>).
        # Gives them _GENGAR_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("gengar-"):
            platform_name = name[len("gengar-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_GENGAR_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
