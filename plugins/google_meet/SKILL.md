...
gengar meet install                 # pip deps + Chromium (transcribe only)
gengar meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
gengar meet auth                    # optional; skips guest-lobby wait
gengar meet setup                   # preflight checks
```

`gengar meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or GENGAR_MEET_REALTIME_KEY in ~/.gengar/.env
```
...
pip install playwright websockets && python -m playwright install chromium
gengar plugins enable google_meet
gengar meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
gengar meet node approve my-mac ws://<mac-ip>:18789 <token>
gengar meet node ping my-mac                   # confirm reachable
```

Run `gengar meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Gengar agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `gengar meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `GENGAR_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `gengar_meet_src`). |
...
Local:
```
$GENGAR_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$GENGAR_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `gengar meet node approve`.
