...
Let the gengar agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where gengar runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `gengar meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `gengar meet` CLI |
...
| `cli.py` | `gengar meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$GENGAR_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `gengar meet node {run,list,approve,remove,status,ping}` |
...
gengar meet install                                      # pip + Chromium
gengar meet setup                                        # preflight
gengar meet auth                                         # optional
gengar meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
gengar meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.gengar/.env
gengar meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
gengar meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
gengar meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.gengar/.env
gengar meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, gengar will **not** switch your system audio input automatically — the
...
gengar plugins enable google_meet
gengar meet node run --display-name my-mac --host 0.0.0.0 --port 18789
