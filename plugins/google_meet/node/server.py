...
Launched by ``gengar meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$GENGAR_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``gengar meet node approve <name> <url> <token>``.
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "gengar-meet-node",
    ) -> None:
        self.host = host
        self.port = port
