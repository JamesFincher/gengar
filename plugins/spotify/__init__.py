...
``_check_spotify_available()`` — when the user has not run ``gengar auth
spotify``, the tools remain registered (so they appear in ``gengar
...
The Spotify auth flow (``gengar auth spotify``), CLI plumbing, and docs
