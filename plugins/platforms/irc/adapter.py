"""
IRC Platform Adapter for Gengar.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Gengar agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "gengar-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Gengar")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Gengar shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: nick_, nick_1, nick_2...
...
def check_requirements() -> bool:
    ...
    # gengar setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `gengar gateway setup` flow for the IRC platform.
    ...
    print_info("Connect Gengar to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #gengar — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.gengar/.env")
    print_info("Restart the gateway for changes to take effect: gengar gateway restart")
