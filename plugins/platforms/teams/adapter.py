"""
Microsoft Teams platform adapter for Gengar.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Gengar"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        gengar_action = data.get("gengar_action") or data.get("hermes_action", "")
        session_key = data.get("session_key", "")

        if not gengar_action or not session_key:
        ...
        choice = choice_map.get(gengar_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="gengar_approve",
                    data={**btn_data_base, "gengar_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="gengar_approve",
                    data={**btn_data_base, "gengar_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="gengar_approve",
                    data={**btn_data_base, "gengar_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="gengar_approve",
                    data={**btn_data_base, "gengar_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Gengar\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.gengar/.env")
    print_info("Install the app in Teams:  teams app install --id <teamsAppId>")
    print_info("Restart the gateway:       gengar gateway restart")
