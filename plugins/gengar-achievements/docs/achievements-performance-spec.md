# Gengar Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: gengar-achievements plugin
...
### `GET /api/plugins/gengar-achievements/achievements`
...
### `POST /api/plugins/gengar-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/gengar-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/gengar-achievements/overview`
...
- Persist plugin data under:
  - `~/.gengar/plugins/gengar-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.gengar/plugins/gengar-achievements/scan_checkpoint.json`
...
- `~/.gengar/plugins/gengar-achievements/`
