...
Auto-tracks and cleans up ephemeral files created during Gengar
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$GENGAR_HOME` and `/tmp/gengar-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `GENGAR_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `GENGAR_HOME` or `/tmp/gengar-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$GENGAR_HOME/disk-cleanup/` is itself excluded
- `$GENGAR_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
