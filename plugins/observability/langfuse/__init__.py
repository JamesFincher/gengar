"""langfuse — Gengar plugin for Langfuse observability.

Traces Gengar conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Gengar plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``gengar plugins enable
observability/langfuse`` or ``gengar tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``gengar tools`` or ~/.gengar/.env):
  GENGAR_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  GENGAR_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  GENGAR_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  GENGAR_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  GENGAR_LANGFUSE_RELEASE     - release/version tag
  GENGAR_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  GENGAR_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  GENGAR_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("GENGAR_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Gengar plugin system —
    ...
    public_key = _env("GENGAR_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("GENGAR_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    if not (public_key and secret_key):
        _LANGFUSE_CLIENT = _INIT_FAILED
        return None

    base_url = _env("GENGAR_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("GENGAR_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("GENGAR_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
    sample_rate = _env("GENGAR_LANGFUSE_SAMPLE_RATE")
    ...
    if sample_rate:
        try:
            ...
            logger.warning("Invalid GENGAR_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("GENGAR_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="Gengar turn",
                tags=["gengar", "langfuse"],
            ):
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Gengar branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Gengar also has a turn-scoped
    ...
    # messages is a list only for legacy Gengar branches that fired
    # pre_llm_call with API messages directly. Current Gengar fires
...
def register(ctx) -> None:
    ...
    # Gengar versions.  pre_api_request / post_api_request fire per API
