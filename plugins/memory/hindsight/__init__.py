...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: gengar)
...
Or via $GENGAR_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Gengar can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $GENGAR_HOME/hindsight/config.json  (profile-scoped)
    ...
    return {
        ...
        "banks": {
            "gengar": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "gengar"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Gengar config."""
    profile = config.get("profile", "gengar")
    return str(profile or "gengar")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Gengar profile name (from agent_identity)
      {workspace} — Gengar workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``gengar-{user}`` with no user becomes ``gengar``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Gengar and the User"
    ...
    def save_config(self, values, hermes_home):
        """Write config to $GENGAR_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
    ...
    def post_setup(self, hermes_home: str, config: dict) -> None:
        ...
        provider_config.setdefault("bank_id", "gengar")
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "gengar"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: gengar-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Gengar and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "gengar"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "gengar"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "gengar", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "gengar")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Gengar and the User")
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Gengar created/used that client on the shared async loop,
