...
Gengar spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.gengar/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p gengar ui start
```
...
Config file: `~/.gengar/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `gengar` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `gengar-{profile}` isolates memory per active Gengar profile. Empty placeholders collapse cleanly (e.g. `gengar-{user}` with no user becomes `gengar`). |
...
The LLM API key is stored in `~/.gengar/.env` as `HINDSIGHT_LLM_API_KEY`.
