...
Handles: gengar honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "gengar" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Gengar profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'gengar honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'gengar honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `gengar update` -- no output, no exceptions.
    """
    try:
        from hermes_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Gengar profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $GENGAR_HOME/honcho.json so each profile/instance gets
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Gengar persistent cross-session memory.")
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'gengar honcho setup' again once you have a key.\n")
            return
    ...
    current_ai = hermes_host.get("aiPeer") or cfg.get("aiPeer", "gengar")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        hermes_host["aiPeer"] = new_ai

    current_workspace = hermes_host.get("workspace") or cfg.get("workspace", "gengar")
    ...
    try:
        ...
        print("  Run: gengar config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    gengar honcho status     -- show full config")
    print("    gengar honcho mode       -- change recall/observation mode")
    print("    gengar honcho tokens     -- tune context and dialectic budgets")
    print("    gengar honcho peer       -- update peer names")
    print("    gengar honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Gengar profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: gengar honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'gengar honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: gengar honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        gengar = hosts.get(_host_key(), {})
        user = gengar.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = gengar.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = gengar.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = gengar.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Gengar' identity in Honcho. Seed with 'gengar honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: gengar honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: gengar honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = gengar.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = gengar.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = gengar.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Gengar asks Honcho's AI peer a question")
        ...
        print("\n  Set with: gengar honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'gengar honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'gengar honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    gengar honcho identity --show        — show both peer representations")
        print("    gengar honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Gengar + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Gengar\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Gengar persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  gengar honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'gengar honcho setup' now?", default="y")
        if answer.lower() in ("y", "yes"):
            ...
            print()
            print("  Run 'gengar honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  gengar honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'gengar'")
        print("  with Honcho configured and no prior session history.")
        print("  (Gengar calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    gengar honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'gengar honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Gengar, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'gengar honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    gengar honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  gengar honcho identity <path/to/SOUL.md>")
    ...
    print("    Gengar:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Gengar:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Gengar:   Honcho observes every message and updates representations")
    ...
    print("    Gengar:   per-session by default — each run gets its own session")
    print("              Map a custom name:  gengar honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. gengar honcho setup              — configure API key (required)")
        print("  2. gengar honcho migrate            — re-run this walkthrough")
    else:
        print("  1. gengar honcho status             — verify Honcho connection")
        print("  2. gengar                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. gengar honcho identity --show    — verify AI peer representation")
        print("  4. gengar honcho tokens             — tune context and dialectic budgets")
        print("  5. gengar honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'gengar memory setup'...\n")
...
def register_cli(subparser) -> None:
    """Build the ``gengar honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``gengar honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to gengar memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Gengar Honcho",
    )
