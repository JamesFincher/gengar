...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/gengar>
...
```bash
gengar honcho setup    # full interactive wizard (cloud or local)
gengar memory setup    # generic picker, also works
```

Or manually:
```bash
gengar config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.gengar/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$GENGAR_HOME/honcho.json` | Profile-local (isolated Gengar instances) |
| 2 | `~/.gengar/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Gengar profile: `gengar` (default) or `gengar.<profile>`.
...
| 4 | `per-session` strategy | Gengar session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`gengar`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`gengar`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-gengar`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening gengar in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Gengar session ID (timestamp + hex). Every `gengar` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Gengar profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`gengar`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Gengar profile: `gengar` (default) or `gengar.<profile>` (e.g. `gengar -p coder` → host key `gengar.coder`).
...
| `GENGAR_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `gengar honcho setup` | Full interactive setup wizard |
| `gengar honcho status` | Show resolved config for active profile |
| `gengar honcho enable` / `disable` | Toggle Honcho for active profile |
| `gengar honcho mode <mode>` | Change recall or observation mode |
| `gengar honcho peer --user <name>` | Update user peer name |
| `gengar honcho peer --ai <name>` | Update AI peer name |
| `gengar honcho tokens --context <N>` | Set context token budget |
| `gengar honcho tokens --dialectic <N>` | Set dialectic max chars |
| `gengar honcho map <name>` | Map current directory to a session name |
| `gengar honcho sync` | Create host blocks for all Gengar profiles |
