...
Resolution order for config file:
  1. $GENGAR_HOME/honcho.json  (instance-local, enables isolated Gengar instances)
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Gengar profile.

    Resolution order:
      1. GENGAR_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``gengar.<profile>``
      3. Fallback: ``"gengar"`` (default profile)
    """
    explicit = os.environ.get("GENGAR_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $GENGAR_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.gengar/honcho.json          (default profile — shared host blocks live here)
    ...
    default_path = Path.home() / ".gengar" / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Gengar system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.gengar
    ...
    def from_global_config(
        ...
        Resolution: $GENGAR_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Gengar profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.gengar block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Gengar session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — Gengar session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Gengar session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'gengar honcho setup' or set HONCHO_API_KEY. "
