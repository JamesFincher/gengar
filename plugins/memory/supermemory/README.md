...
Config file: `$GENGAR_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `gengar` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `gengar-{identity}` → `gengar-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Gengar profile:

```json
{
  "container_tag": "gengar-{identity}"
}
```

For a profile named `coder`, this resolves to `gengar-coder`. The default profile resolves to `gengar-default`. Without `{identity}`, all profiles share the same container.
