...
# Configuration
REPO_URL_SSH="git@github.com:jamesfincher/gengar.git"
REPO_URL_HTTPS="https://github.com/jamesfincher/gengar.git"
GENGAR_HOME="${GENGAR_HOME:-$HOME/.gengar}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${GENGAR_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$GENGAR_INSTALL_DIR"
...
#   code at /usr/local/lib/gengar, command at /usr/local/bin/gengar,
#   data still at /root/.gengar (GENGAR_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.gengar/gengar"
            echo "                   default (root, Linux): /usr/local/lib/gengar"
            echo "  --gengar-home PATH  Data directory (default: ~/.gengar, or \$GENGAR_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Gengar installs the code under"
            echo "  /usr/local/lib/gengar and links the command into"
            echo "  /usr/local/bin/gengar (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$GENGAR_HOME"
            echo "  (default /root/.gengar).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$GENGAR_HOME/gengar are preserved in-place."
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│             ⚕ Gengar Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by James Fincher.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# Decide where the repo checkout + venv live, and where the `gengar` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $GENGAR_HOME/gengar
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $GENGAR_HOME/gengar
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/gengar
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $GENGAR_HOME/gengar — then preserve it)
#
# Always no-op when the user set --dir or $GENGAR_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in GENGAR_HOME.
    if is_termux; then
        INSTALL_DIR="$GENGAR_HOME/gengar"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$GENGAR_HOME/gengar/.git" ]; then
            INSTALL_DIR="$GENGAR_HOME/gengar"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/gengar)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/gengar"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/gengar"
        log_info "  Data:    $GENGAR_HOME (unchanged)"
        return 0
    fi
...
get_hermes_command_path() {
    ...
    if [ -x "$link_dir/gengar" ]; then
        echo "$link_dir/gengar"
    else
        echo "gengar"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/jamesfincher/gengar/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$GENGAR_HOME/node/bin/node" ]; then
        export PATH="$GENGAR_HOME/node/bin:$PATH"
        local found_ver=$("$GENGAR_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Gengar-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.gengar/node/..."
    ...
    # Place into ~/.gengar/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$GENGAR_HOME/node"
    mkdir -p "$GENGAR_HOME"
    mv "$extracted_dir" "$GENGAR_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$GENGAR_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$GENGAR_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$GENGAR_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$GENGAR_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$GENGAR_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.gengar/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Gengar itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="gengar-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Gengar behaves unexpectedly."
                    else
...
install_deps() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        if [ -d "tinker-atropos" ] && [ -f "tinker-atropos/pyproject.toml" ]; then
            log_info "tinker-atropos checkout found — skipping install (optional, for RL training)"
    ...
    # To enable RL tools: uv pip install -e "./tinker-atropos"
    if [ -d "tinker-atropos" ] && [ -f "tinker-atropos/pyproject.toml" ]; then
        log_info "tinker-atropos checkout found — skipping install (optional, for RL training)"
...
setup_path() {
    log_info "Setting up gengar command..."

    if [ "$USE_VENV" = true ]; then
        GENGAR_BIN="$INSTALL_DIR/venv/bin/gengar"
    else
        GENGAR_BIN="$(which gengar 2>/dev/null || echo "")"
        if [ -z "$GENGAR_BIN" ]; then
            log_warn "gengar not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$GENGAR_BIN" ]; then
        log_warn "gengar entry point not found at $GENGAR_BIN"
    ...
    # Create a user-facing shim for the gengar command.
    mkdir -p "$command_link_dir"
    ln -sf "$GENGAR_BIN" "$command_link_dir/gengar"
    log_success "Symlinked gengar → $command_link_display_dir/gengar"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "gengar command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v gengar' \
                >/dev/null 2>&1; then
        ...
        log_info "gengar not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Gengar — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Gengar — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so gengar works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.gengar directory structure (config at top level, code in subdir)
    mkdir -p "$GENGAR_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.gengar/.env (top level, easy to find)
    if [ ! -f "$GENGAR_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$GENGAR_HOME/.env"
            log_success "Created ~/.gengar/.env from template"
        else
            touch "$GENGAR_HOME/.env"
            log_success "Created ~/.gengar/.env"
        fi
    else
        log_info "~/.gengar/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.gengar/config.yaml (top level, easy to find)
    if [ ! -f "$GENGAR_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$GENGAR_HOME/config.yaml"
            log_success "Created ~/.gengar/config.yaml from template"
        fi
    else
        log_info "~/.gengar/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$GENGAR_HOME/SOUL.md" ]; then
        cat > "$GENGAR_HOME/SOUL.md" << 'SOUL_EOF'
# Gengar Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Gengar communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.gengar/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.gengar/"

    # Seed bundled skills into ~/.gengar/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.gengar/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.gengar/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$GENGAR_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$GENGAR_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.gengar/skills/"
        fi
    fi
}
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ubuntu|debian|raspbian|pop|linuxmint|elementary|zorin|kali|parrot)
                ...
                log_info "This is standard Playwright setup — Gengar itself does not require root access."
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (gengar --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'gengar setup' after install."
        return 0
    fi
    ...
    # Run gengar setup using the venv Python directly (no activation needed).
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Gengar to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$GENGAR_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'gengar whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                GENGAR_CMD="$(get_hermes_command_path)"
                $GENGAR_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'gengar whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'gengar gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $GENGAR_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $GENGAR_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: gengar gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: gengar gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $GENGAR_CMD gateway > "$GENGAR_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.gengar/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: gengar gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: gengar gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $GENGAR_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $GENGAR_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $GENGAR_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}gengar${NC}              Start chatting"
    echo -e "   ${GREEN}gengar setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}gengar config${NC}       View/edit configuration"
    echo -e "   ${GREEN}gengar config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}gengar gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}gengar update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'gengar' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'gengar' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'gengar' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
