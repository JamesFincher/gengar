# ============================================================================
# Gengar Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   irm https://raw.githubusercontent.com/jamesfincher/gengar/main/scripts/install.ps1 | iex
#
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$GengarHome = "$env:LOCALAPPDATA\gengar",
    [string]$InstallDir = "$env:LOCALAPPDATA\gengar\gengar"
)
...
$RepoUrlSsh = "git@github.com:jamesfincher/gengar.git"
$RepoUrlHttps = "https://github.com/jamesfincher/gengar.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "│             ⚕ Gengar Installer                    │" -ForegroundColor Magenta
    Write-Host "├─────────────────────────────────────────────────────────┤" -ForegroundColor Magenta
    Write-Host "│  An open source AI agent by James Fincher.              │" -ForegroundColor Magenta
    Write-Host "└─────────────────────────────────────────────────────────┘" -ForegroundColor Magenta
    Write-Host ""
}
...
function Test-Node {
    ...
    $managedNode = "$GengarHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$GengarHome\node;$env:Path"
        Write-Success "Node.js $version found (Gengar-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # Fallback: download binary zip to ~/.gengar/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\gengar-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$GengarHome\node") { Remove-Item -Recurse -Force "$GengarHome\node" }
                Move-Item $extractedDir.FullName "$GengarHome\node"
                $env:Path = "$GengarHome\node;$env:Path"

                $version = & "$GengarHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.gengar/node/"
                $script:HasNode = $true
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/jamesfincher/gengar/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\gengar-$Branch.zip"
                $extractPath = "$env:TEMP\gengar-extract"
...
function Install-Dependencies {
    ...
    if (Test-Path "tinker-atropos\pyproject.toml") {
        ...
        Write-Warn "tinker-atropos not found (run: place an Atropos-compatible checkout at ./tinker-atropos)"
    }
...
function Set-PathVariable {
    Write-Info "Setting up gengar command..."
    ...
    # Add the venv Scripts dir to user PATH so gengar is globally available
    # On Windows, the gengar.exe in venv\Scripts\ has the venv Python baked in
    ...
    # Set GENGAR_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\gengar instead
    # of the Unix default ~/.gengar
    $currentGengarHome = [Environment]::GetEnvironmentVariable("GENGAR_HOME", "User")
    if (-not $currentGengarHome -or $currentGengarHome -ne $GengarHome) {
        [Environment]::SetEnvironmentVariable("GENGAR_HOME", $GengarHome, "User")
        Write-Success "Set GENGAR_HOME=$GengarHome"
    }
    $env:GENGAR_HOME = $GengarHome
    
    # Update current session
    $env:Path = "$hermesBin;$env:Path"
    
    Write-Success "gengar command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.gengar directory structure
    New-Item -ItemType Directory -Force -Path "$GengarHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GengarHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GengarHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GengarHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GengarHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GengarHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GengarHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GengarHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GengarHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$GengarHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.gengar/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.gengar/.env"
        }
    } else {
        Write-Info "~/.gengar/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$GengarHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.gengar/config.yaml from template"
        }
    } else {
        Write-Info "~/.gengar/config.yaml already exists, keeping it"
    }
    
    # Create SOUL.md if it doesn't exist (global persona file)
    $soulPath = "$GengarHome\SOUL.md"
...
"@ | Set-Content -Path $soulPath -Encoding UTF8
        Write-Success "Created ~/.gengar/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.gengar/"
    
    # Seed bundled skills into ~/.gengar/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.gengar/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.gengar/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$GengarHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.gengar/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        try {
            ...
            Write-Warn "TUI npm install failed (gengar --tui may not work)"
        }
        Pop-Location
    }
...
function Start-GatewayIfConfigured {
    ...
    $hermesCmd = "$InstallDir\venv\Scripts\gengar.exe"
    ...
    $whatsappSession = "$GengarHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'gengar whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$GengarHome\logs\gateway.log"
            Start-Process -FilePath $hermesCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$GengarHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: gengar gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: gengar gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$GengarHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$GengarHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$GengarHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$GengarHome\gengar\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   gengar              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   gengar setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   gengar config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   gengar config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   gengar gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   gengar update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/jamesfincher/gengar/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
