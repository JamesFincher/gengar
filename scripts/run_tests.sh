#!/usr/bin/env bash
# Canonical test runner for gengar. Run this instead of calling
...
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.gengar/gengar/venv"; do
...
if [ -z "$VENV" ]; then
  echo "error: no virtualenv found in $REPO_ROOT/.venv, $REPO_ROOT/venv, or $HOME/.gengar/gengar/venv" >&2
  exit 1
fi
...
# Unset GENGAR_* behavioral vars too.
unset GENGAR_YOLO_MODE GENGAR_INTERACTIVE GENGAR_QUIET GENGAR_TOOL_PROGRESS \
      GENGAR_TOOL_PROGRESS_MODE GENGAR_MAX_ITERATIONS GENGAR_SESSION_PLATFORM \
      GENGAR_SESSION_CHAT_ID GENGAR_SESSION_CHAT_NAME GENGAR_SESSION_THREAD_ID \
      GENGAR_SESSION_SOURCE GENGAR_SESSION_KEY GENGAR_GATEWAY_SESSION \
      GENGAR_PLATFORM GENGAR_INFERENCE_PROVIDER GENGAR_MANAGED GENGAR_DEV \
      GENGAR_CONTAINER GENGAR_EPHEMERAL_SYSTEM_PROMPT GENGAR_TIMEZONE \
      GENGAR_REDACT_SECRETS GENGAR_BACKGROUND_NOTIFICATIONS GENGAR_EXEC_ASK \
      GENGAR_HOME_MODE 2>/dev/null || true
