...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs GengarLoop with terminal tool only
...
Requires: pip install gengar[yc-bench]
"""
...
class YCBenchEvalConfig(GengarEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends GengarEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(GengarBaseEnv):
    """
    ...
    a competing built-in agent loop). The GengarLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs GengarLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our GengarLoop.
