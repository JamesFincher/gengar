"""
GengarBaseEnv -- Abstract Base Environment for Gengar + Atropos

Provides the Atropos integration plumbing that all gengar environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via GengarLoop
...
# Ensure the gengar repo root is on sys.path so that imports like
...
# Load API keys from gengar/.env so all environments can access them
...
# Import gengar toolset infrastructure
...
class GengarEnvConfig(BaseEnvConfig):
    """
    Configuration for gengar Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of gengar toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: gengar, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/gengar-results/ "
...
class GengarBaseEnv(BaseEnv):
    """
    Abstract base environment for gengar Atropos integration.
    ...
    name: Optional[str] = "gengar"
    env_config_cls = GengarEnvConfig

    def __init__(
        self,
        config: GengarEnvConfig,
        ...
        # Set terminal environment variables so gengar tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY gengar tool (terminal, file, web,
