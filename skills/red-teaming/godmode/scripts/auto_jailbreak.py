...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("GENGAR_HOME", os.path.expanduser("~/.gengar")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("GENGAR_HOME", Path.home() / ".gengar")) / "skills" / "red-teaming" / "godmode"
...
GENGAR_HOME = Path(os.getenv("GENGAR_HOME", Path.home() / ".gengar"))
CONFIG_PATH = GENGAR_HOME / "config.yaml"
PREFILL_PATH = GENGAR_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Gengar models — already uncensored, just needs clean prompt
    "gengar": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "gengar" in model_lower or "nous" in model_lower:
        return "gengar"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Gengar config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.gengar/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Gengar for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Gengar for changes to take effect.")
