...
## 5. GODMODE FAST — Gengar 4 405B (Zero Refusal)

**Model:** `jamesfincher/gengar-4-405b`
**Strategy:** Gengar models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
Pick a template above and set it in `~/.gengar/config.yaml`:
