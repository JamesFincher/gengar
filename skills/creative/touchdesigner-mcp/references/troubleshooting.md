...
## 5. Gengar Configuration

### Config location

`$GENGAR_HOME/config.yaml` (defaults to `~/.gengar/config.yaml` when `GENGAR_HOME` is unset)
...
Restart the Gengar session for changes to take effect. The MCP connection is
