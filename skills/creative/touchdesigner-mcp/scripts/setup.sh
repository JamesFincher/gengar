...
GENGAR_HOME_DIR="${GENGAR_HOME:-$HOME/.gengar}"
GENGAR_CFG="${GENGAR_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Gengar config has twozero_td MCP entry ──
if [[ ! -f "$GENGAR_CFG" ]]; then
    echo -e " ${FAIL} Gengar config not found at ${GENGAR_CFG}"
    manual_steps+=("Create ${GENGAR_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$GENGAR_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Gengar config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Gengar config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${GENGAR_CFG} manually"); }
    manual_steps+=("Restart Gengar session to pick up config change")
fi
