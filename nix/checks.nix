...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "gengar-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "gengar-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${gengar}/bin/gengar || (echo "FAIL: gengar binary missing"; exit 1)
          test -x ${gengar}/bin/gengar || (echo "FAIL: gengar binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${gengar}/bin/gengar version 2>&1 | grep -qi "gengar" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "gengar-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in gengar gengar gengar-acp; do
            test -x ${gengar}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "gengar-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking gengar --help ==="
          ${gengar}/bin/gengar --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${gengar}/bin/gengar --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "gengar-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${gengar}/share/gengar/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${gengar}/share/gengar/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "GENGAR_BUNDLED_SKILLS" ${gengar}/bin/gengar || \
            (echo "FAIL: GENGAR_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: GENGAR_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "gengar-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${gengar}/share/gengar/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${gengar}/share/gengar/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "GENGAR_BUNDLED_PLUGINS" ${gengar}/bin/gengar || \
            (echo "FAIL: GENGAR_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: GENGAR_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "gengar-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${gengar}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${gengar}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          test -d ${gengar}/ui-tui/node_modules || (echo "FAIL: node_modules missing"; exit 1)
          echo "PASS: node_modules present"

          grep -q "GENGAR_TUI_DIR" ${gengar}/bin/gengar || \
            (echo "FAIL: GENGAR_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: GENGAR_TUI_DIR set in wrapper"
        ...
        # Verify GENGAR_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        gengar-node = pkgs.runCommand "gengar-node-version" { } ''
          set -e
          echo "=== Checking GENGAR_NODE in wrapper ==="
          grep -q "GENGAR_NODE" ${gengar}/bin/gengar || \
            (echo "FAIL: GENGAR_NODE not set in wrapper"; exit 1)
          echo "PASS: GENGAR_NODE present in wrapper"

          GENGAR_NODE=$(sed -n "s/^export GENGAR_NODE='\(.*\)'/\1/p" ${gengar}/bin/gengar)
          test -x "$GENGAR_NODE" || (echo "FAIL: GENGAR_NODE=$GENGAR_NODE not executable"; exit 1)
          echo "PASS: GENGAR_NODE executable at $GENGAR_NODE"

          NODE_MAJOR=$("$GENGAR_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All GENGAR_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify GENGAR_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "gengar-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(GENGAR_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking GENGAR_MANAGED guards ==="
          check_blocked "config set" ${gengar}/bin/gengar config set model foo
          check_blocked "config edit" ${gengar}/bin/gengar config edit
        ...
        in pkgs.runCommand "gengar-extra-python-packages" { } ''
          ...
          if grep -q "PYTHONPATH" ${gengar}/bin/gengar; then
        ...
        in pkgs.runCommand "gengar-config-roundtrip" {
