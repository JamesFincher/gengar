# nix/nixosModules.nix — NixOS module for gengar
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: gengar runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $GENGAR_HOME/.env
# and read by gengar at startup — no container recreation needed for env changes.
#
# Tool resolution: the gengar wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.gengar;
    effectivePackage = if cfg.extraPythonPackages == [ ] then cfg.package
      else cfg.package.override { inherit (cfg) extraPythonPackages; };
    gengar = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-gengar)
    deepConfigType = lib.types.mkOptionType {
      name = "gengar-config-attrs";
      description = "Gengar YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "gengar-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "gengar-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'GENGAR_DOC_EOF'\n${value}\nGENGAR_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "gengar";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/gengar";
    ...
    # gengar user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "gengar-container-entrypoint" ''
      set -eu

      GENGAR_UID="''${GENGAR_UID:?GENGAR_UID must be set}"
      GENGAR_GID="''${GENGAR_GID:?GENGAR_GID must be set}"

      # ── Group: ensure a group with GID=$GENGAR_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$GENGAR_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="gengar"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$GENGAR_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$GENGAR_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$GENGAR_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$GENGAR_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="gengar"
        TARGET_HOME="/home/gengar"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$GENGAR_UID" -g "$GENGAR_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$GENGAR_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$GENGAR_UID:$GENGAR_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure GENGAR_HOME is owned by the target user
      if [ -n "''${GENGAR_HOME:-}" ] && [ -d "$GENGAR_HOME" ]; then
        chown -R "$GENGAR_UID:$GENGAR_GID" "$GENGAR_HOME"
      fi
      ...
      if [ ! -f /var/lib/gengar-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/gengar-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/gengar ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/gengar
        chmod 0440 /etc/sudoers.d/gengar
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$GENGAR_UID" --regid="$GENGAR_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $GENGAR_HOME/.env.
    ...
    # Default: /var/lib/gengar/workspace → /data/workspace.
  ...
  in {
    options.services.gengar = with lib; {
      enable = mkEnableOption "Gengar gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = gengar;
        description = "The gengar package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/gengar";
        description = "State directory. Contains .gengar/ subdir (GENGAR_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Gengar config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $GENGAR_HOME/.env at activation time.
          Gengar reads this file on every startup via load_hermes_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $GENGAR_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $GENGAR_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `gengar gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the gengar user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the gengar plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Gengar discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the gengar CLI to environment.systemPackages and export
          GENGAR_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.gengar symlink to the service
            stateDir. These users are automatically added to the gengar group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.gengar.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the gengar CLI to system PATH and export GENGAR_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.gengar/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.GENGAR_HOME = "${cfg.stateDir}/.gengar";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.gengar.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the gengar user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.gengar: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed gengar
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure gengar is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.gengar        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.gengar/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.gengar/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.gengar/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.gengar/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.gengar/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."gengar-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.gengar
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.gengar ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.gengar ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.gengar -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.gengar/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.gengar/$_subdir"
            chmod 2770 "${cfg.stateDir}/.gengar/$_subdir"
            find "${cfg.stateDir}/.gengar/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.gengar/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.gengar/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.gengar/config.yaml
            chmod 0640 ${cfg.stateDir}/.gengar/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.gengar/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.gengar/.managed
          chmod 0644 ${cfg.stateDir}/.gengar/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.gengar/.container-mode <<'GENGAR_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    hermes_bin=${containerDataDir}/current-package/bin/gengar
    GENGAR_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.gengar/.container-mode
            chmod 0644 ${cfg.stateDir}/.gengar/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.gengar/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.gengar";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.gengar" ]; then
                  rm -f "${symlinkPath}"
                  echo "gengar: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.gengar -> stateDir/.gengar for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.gengar";
                target = "${cfg.stateDir}/.gengar";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "gengar: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.gengar/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.gengar/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.gengar/auth.json
              fi
            ''}
          ''}
          ...
          # Gengar reads $GENGAR_HOME/.env at startup via load_hermes_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.gengar/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'GENGAR_NIX_ENV_EOF'
    ${envFileContent}
    GENGAR_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.gengar/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.gengar/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.gengar/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.gengar = {
          description = "Gengar Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            GENGAR_HOME = "${cfg.stateDir}/.gengar";
            GENGAR_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $GENGAR_HOME/.env by the activation script. load_hermes_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/gengar"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the gengar group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.gengar = {
          description = "Gengar Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              GENGAR_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              GENGAR_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env GENGAR_UID="$GENGAR_UID" \
                --env GENGAR_GID="$GENGAR_GID" \
                --env GENGAR_HOME=${containerDataDir}/.gengar \
                --env GENGAR_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/gengar gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
