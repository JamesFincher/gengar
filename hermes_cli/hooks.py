"""gengar hooks — inspect and manage shell-script hooks.

Usage::

    gengar hooks list
    gengar hooks test <event> [--for-tool X] [--payload-file F]
    gengar hooks revoke <command>
    gengar hooks doctor

Consent records live under ``~/.gengar/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.gengar/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``gengar hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: gengar hooks {list|test|revoke|doctor}")
        print("Run 'gengar hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.gengar/config.yaml.")
        print("See `gengar hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `gengar hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `gengar hooks test` and `gengar hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Gengar wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `gengar hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `gengar hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `gengar hooks doctor`.")
