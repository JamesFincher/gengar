"""
MCP Server Management CLI — ``gengar mcp`` subcommand.

Implements ``gengar mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.gengar/config.yaml under the ``mcp_servers`` key.
"""
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  gengar mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  gengar mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  gengar mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: gengar mcp test " + name)
        return
...
def cmd_mcp_remove(args):
    ...
    # earlier `gengar mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  gengar mcp add <name> --url <endpoint>')
        _info('  gengar mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `gengar mcp remove` + `gengar mcp add` to reconfigure auth.")
        return
...
# ─── gengar mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'gengar mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``gengar mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("gengar mcp serve                              Run as MCP server")
        _info("gengar mcp add <name> --url <endpoint>        Add an MCP server")
        _info("gengar mcp add <name> --command <cmd>         Add a stdio server")
        _info("gengar mcp add <name> --preset <preset>       Add from a known preset")
        _info("gengar mcp remove <name>                      Remove a server")
        _info("gengar mcp list                               List servers")
        _info("gengar mcp test <name>                        Test connection")
        _info("gengar mcp configure <name>                   Toggle tools")
        _info("gengar mcp login <name>                       Re-authenticate OAuth")
        print()
