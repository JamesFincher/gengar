...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or gengar -c.",
    ...
    "Ctrl+Z suspends Gengar to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "gengar -c resumes your most recent CLI session. gengar -c \"project name\" resumes by title.",
    "gengar -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "gengar -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "gengar chat -t web,terminal enables only specific toolsets for a focused session.",
    "gengar chat -s github-pr-workflow preloads a skill at launch.",
    "gengar chat -q \"query\" runs a single non-interactive query and exits.",
    "gengar chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "gengar chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "gengar --yolo bypasses all dangerous command approval prompts for the entire session.",
    "gengar chat --source telegram tags the session for filtering in gengar sessions list.",
    "gengar -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "gengar doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "gengar dump outputs a compact setup summary — great for bug reports.",
    "gengar config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "gengar config edit opens config.yaml in your default editor.",
    "gengar config check scans for missing or stale configuration options.",
    "gengar sessions browse opens an interactive session picker with search.",
    "gengar sessions stats shows session counts by platform and database size.",
    "gengar sessions prune --older-than 30 cleans up old sessions.",
    "gengar skills search react --source skills-sh searches the skills.sh public directory.",
    "gengar skills check scans installed hub skills for upstream updates.",
    "gengar skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "gengar skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "gengar mcp add github --command npx adds MCP servers from the command line.",
    "gengar mcp serve runs Gengar itself as an MCP server for other agents.",
    "gengar auth add lets you add multiple API keys for credential pool rotation.",
    "gengar completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "gengar logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "gengar backup creates a zip backup of your entire Gengar home directory.",
    "gengar profile create coder creates an isolated profile that becomes its own command.",
    "gengar profile create work --clone copies your current config and keys to a new profile.",
    "gengar update syncs new bundled skills to ALL profiles automatically.",
    "gengar gateway install sets up Gengar as a system service (systemd/launchd).",
    "gengar memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "gengar webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: gengar tools disables unused tools, gengar skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "gengar models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as gengar -w).",
    ...
    "GENGAR_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Gengar tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'gengar profile create coder' creates the 'coder' command.",
    "gengar profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Gengar prints a resume command with session ID and stats.",
    "gengar sessions export backup.jsonl exports all sessions for backup or analysis.",
    "gengar -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "gengar skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .gengar/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: gengar cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Gengar runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "gengar gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Gengar via a local macOS server.",
    ...
    "Context files (.gengar.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.gengar/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Gengar loads project context from .gengar.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — gengar tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.gengar/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "gengar chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "gengar plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "gengar acp runs Gengar as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "GENGAR_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "gengar login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "gengar status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.gengar/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Gengar your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.gengar/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Gengar auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.gengar/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at GENGAR_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "GENGAR_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in GENGAR_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.gengar/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'gengar profile create ls' would be rejected.",
    "gengar profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "gengar claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "GENGAR_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via gengar tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.gengar/.env.",
    "Skills from trusted repos (jamesfincher) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.gengar/skills/.hub/quarantine/ holds skills pending security review.",
]
