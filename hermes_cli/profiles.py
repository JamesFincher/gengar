"""
Profile management for multiple isolated Gengar instances.

Each profile is a fully independent GENGAR_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.gengar/profiles/<name>/`` by default.

The "default" profile is ``~/.gengar`` itself — backward compatible,
zero migration needed.

Usage::

    gengar profile create coder          # fresh profile + bundled skills
    gengar profile create coder --clone  # also copy config, .env, SOUL.md, skills
    gengar profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    gengar -p coder chat                 # or via flag
    gengar profile use coder             # set as sticky default
    gengar profile delete coder          # remove profile + alias + service
"""
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    ``~/.gengar`` contains ``profiles/<name>/`` for sibling named profiles.
...
# Directories/files to exclude when exporting the default (~/.gengar) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "gengar",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "gengar", "default", "test", "tmp", "root", "sudo",
})

# Gengar subcommands that cannot be used as profile names/aliases
_GENGAR_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the gengar root, NOT to the current GENGAR_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where GENGAR_HOME points outside
    ``~/.gengar``, profiles live under ``GENGAR_HOME/profiles/`` so
...
def _get_default_hermes_home() -> Path:
    """Return the default (pre-profile) GENGAR_HOME path.

    In standard deployments this is ``~/.gengar``.
    In Docker/custom deployments where GENGAR_HOME is outside ``~/.gengar``
    (e.g. ``/opt/data``), returns GENGAR_HOME directly.
    """
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.gengar
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its GENGAR_HOME directory."""
    if name == "default":
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, gengar subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _GENGAR_SUBCOMMANDS:
        return f"'{name}' conflicts with a gengar subcommand"
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.gengar)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.gengar/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches GENGAR_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "GENGAR_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.gengar).\n"
            "To remove everything, use: gengar uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set GENGAR_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("GENGAR_HOME")
    try:
        os.environ["GENGAR_HOME"] = str(profile_dir)
        ...
        if old_home is not None:
            os.environ["GENGAR_HOME"] = old_home
        elif "GENGAR_HOME" in os.environ:
            del os.environ["GENGAR_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.gengar/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: gengar profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from GENGAR_HOME.

    Returns ``"default"`` if GENGAR_HOME is not set or points to ``~/.gengar``.
    Returns the profile name if GENGAR_HOME points into ``~/.gengar/profiles/<name>``.
    Returns ``"custom"`` if GENGAR_HOME is set to an unrecognized path.
    """
    from hermes_constants import get_hermes_home
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.gengar itself — its parent is ~/ and its
        # directory name is ".gengar", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: gengar profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.gengar itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.gengar). "
            "Specify a different name: gengar profile import <archive> --name <name>"
        )
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"gengar.{old_name}"
    new_host = f"gengar.{new_name}"
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for gengar profile names."""
    return '''# Gengar profile completion
# Add to ~/.bashrc: eval "$(gengar completion bash)"

_hermes_profiles() {
    local profiles_dir="$HOME/.gengar/profiles"
...
complete -F _hermes_completion gengar
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for gengar profile names."""
    return '''#compdef gengar
# Gengar profile completion
# Add to ~/.zshrc: eval "$(gengar completion zsh)"

_hermes() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.gengar/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.gengar/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a GENGAR_HOME path string.

    Called early in the CLI entry point, before any gengar modules
    are imported, to set the GENGAR_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: gengar profile create {profile_name}"
        )
