"""Slash command definitions and autocomplete for the Gengar CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Gengar config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Gengar works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Gengar is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.gengar/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Gengar to the latest version", "Info",
               gateway_only=True),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    Telegram command menu, in Slack's ``/gengar`` subcommand mapping, and
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``gengar skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``gengar skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``gengar skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/gengar <verb>`` subcommand.
    ...
    can still reach them via ``/gengar <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/gengar`` is always reserved as the first entry so the
    legacy ``/gengar <subcommand>`` form keeps working for anything that
    ...
    # Reserve /gengar as the catch-all top-level command.
    entries.append(("gengar", "Talk to Gengar or run a subcommand", "[subcommand] [args]"))
    seen.add("gengar")
...
def slack_app_manifest(request_url: str = "https://gengar.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /gengar handler.

    Maps both canonical names and aliases so /gengar bg do stuff works
    the same as /gengar background do stuff.

    Plugin-registered slash commands are included so ``/gengar <plugin-cmd>``
