...
def auth_add_command(args) -> None:
    ...
    if provider == "openai-codex":
        # Clear any existing suppression marker so a re-link after `gengar auth
    ...
    raise SystemExit(f"`gengar auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Gengar reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `gengar auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `gengar auth` is called bare."""
