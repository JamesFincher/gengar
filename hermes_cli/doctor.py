"""
Doctor command for gengar CLI.

Diagnoses issues with Gengar setup.
"""
...
GENGAR_HOME = get_hermes_home()
_DHH = display_hermes_home()  # user-facing display path (e.g. ~/.gengar or ~/.gengar/profiles/coder)

# Load environment variables from ~/.gengar/.env so API key checks work
...
from hermes_cli.models import _GENGAR_USER_AGENT
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.gengar/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `gengar`.
    os.environ.setdefault("GENGAR_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 Gengar Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.gengar/.env (primary location for user config)
    env_path = GENGAR_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'gengar setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'gengar setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'gengar setup' to create one")
                issues.append("Run 'gengar setup' to create .env")
    
    # Check ~/.gengar/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = GENGAR_HOME / 'config.yaml'
            ...
            if provider and provider != "auto":
                if catalog_provider is None or (
                    ...
                    issues.append(
                        ...
                        f"Fix: run 'gengar config set model.provider <valid_provider>'"
                    )
                    ...
                    if pconfig and getattr(pconfig, "auth_type", "") == "api_key":
                        ...
                        if not configured:
                            check_fail(
                                ...
                                "(check ~/.gengar/.env or run 'gengar setup')",
                            )
                            issues.append(
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'gengar setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'gengar config set model.provider <name>'"
                            )
                except Exception:
                    pass
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'gengar setup' to migrate config")
                else:
                    issues.append("Run 'gengar doctor --fix' or 'gengar setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'gengar doctor --fix'")
        except Exception:
            pass
    ...
    if _safe_which("codex"):
        ...
        # Native OAuth uses Gengar' own device-code flow — the Codex CLI is
        # only needed if you want to import existing tokens from
        # ~/.codex/auth.json.  Downgrade to info so users running
        # `gengar auth openai-codex` aren't told they're missing something.
        check_info(
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Gengar a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Gengar Persona\n\n"
                "<!-- Edit this file to customize how Gengar communicates. -->\n\n"
                "You are Gengar, a helpful AI assistant.\n",
                encoding="utf-8",
            )
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'gengar doctor --fix' to checkpoint")
    ...
    # =========================================================================
    # Check: Command installation (gengar bin symlink)
    # =========================================================================
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "gengar"
        ...
        _cmd_link = _cmd_link_dir / "gengar"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(gengar not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/gengar → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/gengar points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/gengar → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/gengar — run 'gengar doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/gengar exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/gengar not found",
                    "(gengar command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/gengar → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/gengar symlink — run 'gengar doctor --fix'")
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            check_fail("vercel SDK not installed", "(pip install 'gengar[vercel]')")
            issues.append("Install the Vercel optional dependency: pip install 'gengar[vercel]'")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                issues.append(
                    ...
                    "Fix: run 'gengar config set model.provider <provider>' to switch providers, "
    ...
    if tinker_dir.exists() and (tinker_dir / "pyproject.toml").exists():
        ...
        check_warn("tinker-atropos not found", "(run: place an Atropos-compatible checkout at ./tinker-atropos)")
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'gengar setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    hub_dir = GENGAR_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: gengar skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = GENGAR_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: gengar memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: gengar memory setup")
                issues.append("No Honcho API key — run 'gengar memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run gengar memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: gengar memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: gengar memory setup")
        except Exception as _e:
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "gengar -p" in content:
                            _m = _re.search(r"gengar -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'gengar doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
