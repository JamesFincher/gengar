"""gengar webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.gengar/webhook_subscriptions.json and are
...
def _setup_hint() -> str:
  ...
  Then start the gateway: gengar gateway run
"""
...
def webhook_command(args):
    """Entry point for 'gengar webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: gengar webhook {subscribe|list|remove|test}")
        print("Run 'gengar webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (gengar gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: gengar webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from gengar webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (gengar gateway run)")
