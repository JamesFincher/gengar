"""
Single source of truth for provider identity in Gengar.
...
2. **Gengar overlays** — transport type, auth patterns, aggregator flags,
...
# -- Gengar overlay ----------------------------------------------------------
# Gengar-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class GengarOverlay:
    """Gengar-specific provider metadata layered on top of models.dev."""
...
GENGAR_OVERLAYS: Dict[str, GengarOverlay] = {
    "openrouter": GengarOverlay(
    ...
    "openai-codex": GengarOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": GengarOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="GENGAR_QWEN_BASE_URL",
    ),
    "google-gemini-cli": GengarOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": GengarOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": GengarOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": GengarOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": GengarOverlay(
    ...
    "kimi-for-coding": GengarOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": GengarOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": GengarOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": GengarOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": GengarOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": GengarOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": GengarOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": GengarOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "vercel": GengarOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": GengarOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": GengarOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": GengarOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": GengarOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": GengarOverlay(
        transport="openai_chat",
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "gengar", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Gengar overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Gengar overlay
    ...
    overlay = GENGAR_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + gengar extra
    ...
    if overlay is not None:
        # Gengar-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in GENGAR_OVERLAYS
    if provider == "bedrock":
