"""Shell completion script generation for gengar CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# Gengar bash completion
# Add to ~/.bashrc:
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef gengar
# Gengar zsh completion
# Add to ~/.zshrc:
...
{top_cmds_str}
            ...
            _describe 'gengar command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Gengar fish completion",
        "# Add to your config:",
        "#   gengar completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __hermes_profiles",
        "    echo default",
        "    if test -d $HOME/.gengar/profiles",
        "        ls $HOME/.gengar/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c gengar -f -s p -l profile"
