"""Persistent session goals — the Ralph loop for Gengar.

A goal is a free-form user objective that stays active across turns. After
each turn completes, a small judge call asks an auxiliary model "is this
goal satisfied by the assistant's last response?". If not, Gengar feeds a
...
- When a real user message arrives mid-loop it preempts the continuation
  ...
  will say ``done``).
- This module has zero hard dependency on ``cli.GengarCLI`` or the gateway
...
def _get_session_db() -> Optional[Any]:
    """Return a SessionDB instance for the current GENGAR_HOME.
