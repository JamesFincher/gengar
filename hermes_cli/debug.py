"""``gengar debug`` debug tools for Gengar.

Currently supports:
    gengar debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.gengar/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[gengar debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.gengar/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``gengar debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``gengar debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Gengar version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `gengar debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "gengar/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``gengar debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.gengar/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``gengar debug share`` also runs an opportunistic sweep as a fallback
...
def _delete_hint(url: str) -> str:
    ...
    paste_id = _extract_paste_id(url)
    if paste_id:
        return f"gengar debug delete {url}"
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----GengarDebugBoundary9f3c"
...
def _capture_dump() -> str:
    """Run ``gengar dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``gengar dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    if redact:
        logger.info(
            "gengar debug share: applied force-mode redaction to log snapshots before upload"
        )
    ...
    print(f"To delete now:  gengar debug delete <url>")

    print(f"\nShare these links with the Gengar team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: gengar debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'gengar debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``gengar debug`` call.
    ...
    # best-effort — any failure is swallowed so ``gengar debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: gengar debug <command>")
        print()
        print("Commands:")
