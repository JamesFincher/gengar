"""``gengar logs`` — view and filter Gengar log files.
...
Usage examples::
    ...
    gengar logs                    # last 50 lines of agent.log
    gengar logs -f                 # follow agent.log in real time
    gengar logs errors             # last 50 lines of errors.log
    gengar logs gateway -n 100    # last 100 lines of gateway.log
    gengar logs --level WARNING    # only WARNING+ lines
    gengar logs --session abc123   # filter by session ID substring
    gengar logs --component tools  # only tool-related lines
    gengar logs --since 1h         # lines from the last hour
    gengar logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print(f"(Logs are created when Gengar runs — try 'gengar chat' first)")
        sys.exit(1)
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet — run 'gengar chat' to generate logs)")
