# Gengar Security Policy

This document outlines the security protocols, trust model, and deployment hardening guidelines for the **Gengar** project.

## 1. Vulnerability Reporting

Gengar does **not** operate a bug bounty program. Security issues should be reported via [GitHub Security Advisories (GHSA)](https://github.com/jamesfincher/gengar/security/advisories/new) or by emailing **security@jamesfincher.com**. Do not open public issues for security vulnerabilities.
...
- **Environment:** Output of `gengar version`, commit SHA, OS, and Python version.
...
The core assumption is that Gengar is a **personal agent** with one trusted operator.
...
- **Trusted State Access:** Reports that require pre-existing write access to `~/.gengar/`, `.env`, or `config.yaml` (these are operator-owned files).
...
- **File permissions:** Run as non-root (the Docker image uses UID 10000); protect credentials with `chmod 600 ~/.gengar/.env` on local installs.
...
### Skills & Supply Chain
- **Skill installation:** Review Skills Guard reports (`tools/skills_guard.py`) before installing third-party skills. The audit log at `~/.gengar/skills/.hub/audit.log` tracks every install and removal.
...
### Credential Storage
- API keys and tokens belong exclusively in `~/.gengar/.env` — never in `config.yaml` or checked into version control.
...
- **Communication:** All updates occur via the GHSA thread or email correspondence with security@jamesfincher.com.
