...
class TestGengarTimeNow:
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["GENGAR_TIMEZONE"] = "UTC"
        result = hermes_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["GENGAR_TIMEZONE"] = "America/New_York"
        result = hermes_time.now()
        assert result.tzinfo is not None
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["GENGAR_TIMEZONE"] = "Mars/Olympus_Mons"
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["GENGAR_TIMEZONE"] = "Europe/London"
    ...
    def test_returns_none_for_invalid(self):
        os.environ["GENGAR_TIMEZONE"] = "Not/A/Timezone"
        tz = hermes_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When GENGAR_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) GENGAR_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("GENGAR_TIMEZONE=" + os.environ.get("GENGAR_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "GENGAR_TIMEZONE=NOT_SET" in result["output"], (
            "GENGAR_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When GENGAR_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("GENGAR_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        absolute time when system-local tz != Gengar tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_hermes_tz(self):
        """Already-aware datetimes should be normalized to Gengar tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Gengar tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Gengar tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Gengar is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Gengar tz
        ...
        # Use a Gengar timezone far behind UTC so that the numeric wall time
        ...
        os.environ["GENGAR_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_hermes_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Gengar timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["GENGAR_TIMEZONE"] = "US/Eastern"
        _reset_hermes_time_cache()
