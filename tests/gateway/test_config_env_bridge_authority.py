...
Guards against the 60-vs-500 bug where a stale `.env GENGAR_MAX_ITERATIONS=60`
...
def _run_gateway_import(hermes_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    script = textwrap.dedent(
        ...
        for k in (
            ...
            "GENGAR_AGENT_TIMEOUT",
            "GENGAR_AGENT_TIMEOUT_WARNING",
            "GENGAR_GATEWAY_BUSY_INPUT_MODE",
            "GENGAR_TIMEZONE",
        ):
            v = os.environ.get(k)
            if v is not None:
    ...
    env["GENGAR_HOME"] = str(hermes_home)
...
def test_config_max_turns_wins_over_stale_env(hermes_home: Path) -> None:
    ...
    _write_env(hermes_home, {"GENGAR_MAX_ITERATIONS": "60"})

    env = _run_gateway_import(hermes_home, initial_env={})

    assert env.get("GENGAR_MAX_ITERATIONS") == "500", (
        f"expected config.yaml max_turns=500 to win; got {env.get('GENGAR_MAX_ITERATIONS')!r}. "
...
def test_config_gateway_timeout_wins_over_stale_env(hermes_home: Path) -> None:
    ...
    _write_env(hermes_home, {
        "GENGAR_AGENT_TIMEOUT": "60",
        "GENGAR_AGENT_TIMEOUT_WARNING": "30",
    })

    env = _run_gateway_import(hermes_home, initial_env={})

    assert env.get("GENGAR_AGENT_TIMEOUT") == "1800"
    assert env.get("GENGAR_AGENT_TIMEOUT_WARNING") == "900"
...
def test_config_display_busy_input_mode_wins_over_stale_env(hermes_home: Path) -> None:
    ...
    _write_env(hermes_home, {"GENGAR_GATEWAY_BUSY_INPUT_MODE": "queue"})

    env = _run_gateway_import(hermes_home, initial_env={})

    assert env.get("GENGAR_GATEWAY_BUSY_INPUT_MODE") == "interrupt"


def test_config_timezone_wins_over_stale_env(hermes_home: Path) -> None:
    _write_config(hermes_home, timezone="America/Los_Angeles")
    _write_env(hermes_home, {"GENGAR_TIMEZONE": "UTC"})

    env = _run_gateway_import(hermes_home, initial_env={})

    assert env.get("GENGAR_TIMEZONE") == "America/Los_Angeles"
...
def test_env_value_survives_when_config_omits_key(hermes_home: Path) -> None:
    ...
    _write_env(hermes_home, {"GENGAR_MAX_ITERATIONS": "123"})

    env = _run_gateway_import(hermes_home, initial_env={})

    assert env.get("GENGAR_MAX_ITERATIONS") == "123"
