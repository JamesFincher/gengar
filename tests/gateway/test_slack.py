...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.gengar."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /gengar, /btw, /stop,
        ...
        for expected in ("/gengar", "/btw", "/stop", "/model", "/help"):
...
class TestSlashCommands:
    ...
    # instead of as /gengar subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /gengar btw."""
        command = {
            "command": "/btw",
    ...
    async def test_legacy_hermes_prefix_still_works(self, adapter):
        """Backward compat: /gengar btw foo must still route to /btw foo.

        Old workspace manifests only declared /gengar as the single slash.
    ...
    async def test_legacy_hermes_freeform_question(self, adapter):
        """/gengar <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/gengar",
...
class TestSlashEphemeralAck:
    ...
    async def test_legacy_hermes_slash_stashes_context(self, adapter):
        """Legacy /gengar <subcommand> also stashes context."""
        command = {
            "command": "/gengar",
            "text": "help",
            "user_id": "U_H",
    ...
    async def test_freeform_hermes_question_does_not_stash_context(self, adapter):
        """Free-form /gengar <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/gengar",
