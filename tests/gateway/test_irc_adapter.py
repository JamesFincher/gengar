...
class TestIRCProtocolHelpers:
    ...
    def test_parse_numeric_reply(self):
        msg = _parse_irc_message(":server 001 gengar-bot :Welcome to IRC")
        assert msg["prefix"] == "server"
        assert msg["command"] == "001"
        assert msg["params"] == ["gengar-bot", "Welcome to IRC"]

    def test_parse_nick_collision(self):
        msg = _parse_irc_message(":server 433 * gengar-bot :Nickname is already in use")
...
class TestIRCAdapterInit:
    ...
    def test_init_from_config_extra(self, monkeypatch):
        ...
        assert adapter.nickname == "gengar"
        assert adapter.channel == "#gengar-dev"
...
class TestIRCAdapterMessageParsing:
    ...
    async def test_handle_welcome(self, adapter):
        ...
        await adapter._handle_line(":server 001 gengar :Welcome to IRC")
    ...
    async def test_handle_nick_collision(self, adapter):
        ...
        await adapter._handle_line(":server 433 * gengar :Nickname in use")
        assert adapter._current_nick == "gengar_"
        sent = writer.write.call_args[0][0]
        assert b"NICK gengar_" in sent
    ...
    async def test_handle_addressed_channel_message(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG #test :gengar: hello there")
    ...
    async def test_handle_dm(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG gengar :private message")
        assert len(dispatched) == 1
    ...
    async def test_ignores_own_messages(self, adapter):
        ...
        await adapter._handle_line(":gengar!bot@host PRIVMSG #test :my own msg")
        assert len(dispatched) == 0
    ...
    async def test_ctcp_action_converted(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG gengar :\x01ACTION waves\x01")
        assert len(dispatched) == 1
    ...
    async def test_allowed_users_case_insensitive(self, monkeypatch):
        ...
        await adapter._handle_line(":admin!u@host PRIVMSG #test :gengar: hello")
        assert len(dispatched) == 1
    ...
    async def test_unauthorized_user_blocked(self, monkeypatch):
        ...
        await adapter._handle_line(":eve!u@host PRIVMSG #test :gengar: hello")
        assert len(dispatched) == 0
    ...
    async def test_nick_collision_retry(self, adapter):
        ...
        await adapter._handle_line(":server 433 * gengar :Nickname in use")
        assert adapter._current_nick == "gengar_"
        await adapter._handle_line(":server 433 * gengar_ :Nickname in use")
        assert adapter._current_nick == "gengar_1"
        await adapter._handle_line(":server 433 * gengar_1 :Nickname in use")
        assert adapter._current_nick == "gengar_2"
