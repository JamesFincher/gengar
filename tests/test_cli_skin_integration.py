...
from cli import GengarCLI, _build_compact_banner, _rich_text_from_ansi
from hermes_cli.skin_engine import get_active_skin, set_active_skin


def _make_cli_stub():
    cli = GengarCLI.__new__(GengarCLI)
...
class TestCompactBannerSkinIntegration:
    def test_default_compact_banner_keeps_legacy_nous_hermes_branding(self):
        ...
        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch.dict(_build_compact_banner.__globals__, {"format_banner_version_label": lambda: "Gengar v0.1.0 (test)"}):
            banner = _build_compact_banner()

        assert "NOUS GENGAR" in banner
    ...
    def test_poseidon_compact_banner_uses_skin_branding_instead_of_nous_hermes(self):
        ...
        assert "NOUS GENGAR" not in banner
    ...
    def test_compact_banner_shows_version_label(self):
        ...
        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch.dict(_build_compact_banner.__globals__, {"format_banner_version_label": lambda: "Gengar v1.0 (test) · upstream abc12345"}):
            banner = _build_compact_banner()
