...
- Unlimited timeout (GENGAR_CRON_TIMEOUT=0)
- Backward compat: GENGAR_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """GENGAR_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """GENGAR_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("GENGAR_CRON_TIMEOUT", "1200")
        raw = os.getenv("GENGAR_CRON_TIMEOUT", "").strip()
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """GENGAR_CRON_TIMEOUT=0 yields None (unlimited)."""
        monkeypatch.setenv("GENGAR_CRON_TIMEOUT", "0")
        raw = os.getenv("GENGAR_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
        _cron_inactivity_limit = _cron_timeout if _cron_timeout > 0 else None
    ...
    def test_timeout_invalid_value_falls_back_to_default(self, monkeypatch):
        """GENGAR_CRON_TIMEOUT=abc should fall back to 600s, not raise ValueError."""
        monkeypatch.setenv("GENGAR_CRON_TIMEOUT", "abc")
        raw = os.getenv("GENGAR_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
    ...
    def test_timeout_empty_string_uses_default(self, monkeypatch):
        """GENGAR_CRON_TIMEOUT='' (empty) should use the 600s default."""
        monkeypatch.setenv("GENGAR_CRON_TIMEOUT", "")
        raw = os.getenv("GENGAR_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
        assert _cron_timeout == 600.0
