...
- Path resolution (absolute, relative to GENGAR_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp GENGAR_HOME."""
    ...
    monkeypatch.setattr(jobs_mod, "GENGAR_DIR", hermes_home)
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.gengar/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("GENGAR_SESSION_PLATFORM") is None
        assert os.environ.get("GENGAR_SESSION_CHAT_ID") is None
        assert os.environ.get("GENGAR_SESSION_CHAT_NAME") is None
