...
class TestGatewayPersonalityNone:
    ...
    async def test_empty_personality_list_uses_profile_display_path(self, tmp_path):
        ...
        with patch("gateway.run._hermes_home", tmp_path), \
             patch("hermes_constants.display_hermes_home", return_value="~/.gengar/profiles/coder"):
            event = self._make_event("")
            result = await runner._handle_personality_command(event)

        assert result == "No personalities configured in `~/.gengar/profiles/coder/config.yaml`"
...
class TestPersonalityDictFormat:
    ...
    def test_resolve_prompt_dict_no_tone_no_style(self):
        from cli import GengarCLI
        result = GengarCLI._resolve_personality_prompt({
    ...
    def test_resolve_prompt_string(self):
        from cli import GengarCLI
        result = GengarCLI._resolve_personality_prompt("You are helpful.")
        assert result == "You are helpful."
