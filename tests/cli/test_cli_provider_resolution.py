...
def test_hermes_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    ...
    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in GengarCLI.__init__")

    monkeypatch.setattr("hermes_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("hermes_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.GengarCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_cli_prefers_config_provider_over_stale_env_override(monkeypatch):
    ...
    shell = cli.GengarCLI(model="fireworks/minimax-m2p5", compact=True, max_turns=1)
...
def test_codex_provider_replaces_incompatible_default_model(monkeypatch):
    ...
    shell = cli.GengarCLI(compact=True, max_turns=1)
...
def test_codex_provider_preserves_explicit_codex_model(monkeypatch):
    ...
    shell = cli.GengarCLI(model="gpt-5.1-codex-mini", compact=True, max_turns=1)
...
def test_codex_provider_strips_provider_prefix_from_model(monkeypatch):
    ...
    shell = cli.GengarCLI(model="openai/gpt-5.3-codex", compact=True, max_turns=1)
...
def test_cmd_model_forwards_nous_login_tls_options(monkeypatch):
    ...
    hermes_main.cmd_model(
        SimpleNamespace(
            ...
            inference_url="https://inference.jamesfincher.com/v1",
            client_id="gengar-local",
