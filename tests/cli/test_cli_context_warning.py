...
def _isolate(tmp_path, monkeypatch):
    """Isolate GENGAR_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal GengarCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = GengarCLI.__new__(GengarCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["gengar-core"]
        obj.compact = False
