...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("gengar/gengar-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned gengar/* and pr-* branches."""

    def test_prunes_orphaned_hermes_branch(self, git_repo):
        """gengar/gengar-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "gengar/gengar-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "gengar/gengar-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "gengar/gengar-deadbeef" in result.stdout
        ...
        assert "gengar/gengar-deadbeef" in orphaned
        ...
        assert "gengar/gengar-deadbeef" not in result.stdout
