...
def hermes_home(monkeypatch):
    """Isolate GENGAR_HOME for ``reload_skills`` tests.
    ...
    module-level ``GENGAR_HOME`` / ``SKILLS_DIR`` constants in place so the
    isolation is local to this fixture's scope.
    """
    td = tempfile.mkdtemp(prefix="gengar-reload-skills-")
    monkeypatch.setenv("GENGAR_HOME", td)
    home = Path(td)
    ...
    monkeypatch.setattr(_st, "GENGAR_HOME", home, raising=False)
