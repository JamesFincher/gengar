...
def test_round_robin_strategy_rotates_priorities(tmp_path, monkeypatch):
    ...
    config_path = tmp_path / "gengar" / "config.yaml"
...
def test_mark_exhausted_and_rotate_persists_status(tmp_path, monkeypatch):
    ...
    auth_payload = json.loads((tmp_path / "gengar" / "auth.json").read_text())
...
def test_load_pool_prefers_dotenv_over_stale_os_environ(tmp_path, monkeypatch):
    ...
    from a parent shell) must NOT shadow the fresh key in ~/.gengar/.env when
    ...
    # User edited ~/.gengar/.env with the fresh key
...
def test_load_pool_falls_back_to_os_environ_when_dotenv_empty(tmp_path, monkeypatch):
    """When ~/.gengar/.env does not define OPENROUTER_API_KEY (typical Docker /
...
def test_get_custom_provider_pool_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("GENGAR_HOME", str(tmp_path / "gengar"))
    (tmp_path / "gengar").mkdir(parents=True, exist_ok=True)
    import yaml
    config_path = tmp_path / "gengar" / "config.yaml"
    config_path.write_text(yaml.dump({
        "custom_providers": [
        ...
        tmp_path,
        {
            ...
            "providers": {
                "nous": {
                    ...
                    "inference_base_url": "https://inference.jamesfincher.com/v1",
                    "token_type": "Bearer",
...
def test_sync_codex_entry_from_auth_store_adopts_newer_tokens(tmp_path, monkeypatch):
    ...
    # Simulate `gengar auth openai-codex` replacing the token pair on disk.
...
def test_codex_exhausted_entry_recovers_via_auth_store_sync(tmp_path, monkeypatch):
    ...
    rate-limit reset the user ran `gengar model` to reauth, but the pool
    ...
    # Simulate `gengar model` / `gengar auth` refreshing the tokens.
