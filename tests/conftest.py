"""Shared fixtures for the gengar test suite.
...
2. **Isolated GENGAR_HOME.** GENGAR_HOME points to a per-test tempdir so
   code reading ``~/.gengar/*`` via ``get_hermes_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".gengar"`` instead of the canonical
   ``get_hermes_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No GENGAR_SESSION_* inheritance** — the agent's current gateway
...
# GENGAR_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_GENGAR_BEHAVIORAL_VARS = frozenset({
    "GENGAR_YOLO_MODE",
    "GENGAR_INTERACTIVE",
    "GENGAR_QUIET",
    ...
    "GENGAR_SESSION_PLATFORM",
    "GENGAR_SESSION_CHAT_ID",
    "GENGAR_SESSION_CHAT_NAME",
    "GENGAR_SESSION_THREAD_ID",
    "GENGAR_SESSION_SOURCE",
    "GENGAR_SESSION_KEY",
    "GENGAR_GATEWAY_SESSION",
    "GENGAR_PLATFORM",
    "GENGAR_MODEL",
    "GENGAR_INFERENCE_MODEL",
    "GENGAR_INFERENCE_PROVIDER",
    "GENGAR_TUI_PROVIDER",
    "GENGAR_MANAGED",
    "GENGAR_DEV",
    "GENGAR_CONTAINER",
    ...
    "GENGAR_TIMEZONE",
    "GENGAR_REDACT_SECRETS",
    "GENGAR_BACKGROUND_NOTIFICATIONS",
    "GENGAR_EXEC_ASK",
    "GENGAR_HOME_MODE",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and GENGAR_HOME to per-test tempdirs so code that
    reads ``~/.gengar/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral GENGAR_* vars that could change test semantics.
    for name in _GENGAR_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect GENGAR_HOME to a per-test tempdir. Code that reads
    #    ``~/.gengar/*`` via ``get_hermes_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.gengar/*`` via
    #    ``Path.home() / ".gengar"`` instead of ``get_hermes_home()``
    ...
    monkeypatch.setenv("GENGAR_HOME", str(fake_hermes_home))
    ...
    #    ~/.gengar/plugins/ (which, per step 3, is now empty — but the
...
@pytest.fixture()
def mock_config():
    """Return a minimal gengar config dict suitable for unit tests."""
    return {
