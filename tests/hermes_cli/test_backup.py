"""Tests for gengar backup and import commands."""
...
def _make_hermes_tree(root: Path) -> None:
    """Create a realistic ~/.gengar directory structure for testing."""
    ...
    # gengar repo (should be EXCLUDED)
    (root / "gengar").mkdir(exist_ok=True)
    (root / "gengar" / "run_agent.py").write_text("# big file\n")
    (root / "gengar" / ".git").mkdir()
    (root / "gengar" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestBackup:
    ...
    def test_excludes_gengar_checkout(self, tmp_path, monkeypatch):
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "gengar" in n]
            assert agent_files == [], f"gengar files leaked into backup: {agent_files}"
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/gengar-backup-*.zip."""
        hermes_home = tmp_path / ".gengar"
        hermes_home.mkdir()
        ...
        zips = list(tmp_path.glob("gengar-backup-*.zip"))
        assert len(zips) == 1
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Gengar backup."""
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into gengar home."""
        hermes_home = tmp_path / ".gengar"
        hermes_home.mkdir()
        monkeypatch.setenv("GENGAR_HOME", str(hermes_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_strips_hermes_prefix(self, tmp_path, monkeypatch):
        """Import strips .gengar/ prefix if all entries share it."""
        hermes_home = tmp_path / ".gengar"
        hermes_home.mkdir()
        monkeypatch.setenv("GENGAR_HOME", str(hermes_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".gengar/config.yaml": "model: test\n",
            ".gengar/skills/a/SKILL.md": "# A\n",
        })
    ...
    def test_rejects_non_hermes_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a gengar backup."""
        hermes_home = tmp_path / ".gengar"
        hermes_home.mkdir()
        monkeypatch.setenv("GENGAR_HOME", str(hermes_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        # traversal file should NOT exist outside gengar home
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".gengar"
        src_home.mkdir(parents=True)
        _make_hermes_tree(src_home)

        monkeypatch.setenv("GENGAR_HOME", str(src_home))
        ...
        dst_home = tmp_path / "dest" / ".gengar"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("GENGAR_HOME", str(dst_home))
        ...
        # gengar should NOT be present
        assert not (dst_home / "gengar").exists()
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without gengar markers fails validation."""
        import io
        from hermes_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_hermes(self):
        """Detects .gengar/ prefix wrapping all entries."""
        import io
        from hermes_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".gengar/config.yaml", "test")
            zf.writestr(".gengar/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".gengar/"
...
class TestBackupEdgeCases:
    def test_nonexistent_hermes_home(self, tmp_path, monkeypatch):
        """Backup exits when gengar home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".gengar"
        monkeypatch.setenv("GENGAR_HOME", str(fake_home))
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("gengar-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_hermes_home(self, tmp_path, monkeypatch):
        """Backup handles empty gengar home (no files to back up)."""
        hermes_home = tmp_path / ".gengar"
        hermes_home.mkdir()
    ...
    def test_skips_output_zip_inside_hermes(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside gengar root."""
        hermes_home = tmp_path / ".gengar"
        hermes_home.mkdir()
        (hermes_home / "config.yaml").write_text("model: test\n")
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert "gengar -p coder" in coder_wrapper
...
class TestQuickSnapshot:
    ...
    def hermes_home(self, tmp_path):
        """Create a fake GENGAR_HOME with critical state files."""
...
# ---------------------------------------------------------------------------
# Pre-update backup (gengar update safety net)
# ---------------------------------------------------------------------------

class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``gengar update``
    runs before touching anything."""

    @pytest.fixture
    def hermes_home(self, tmp_path):
        root = tmp_path / ".gengar"
        root.mkdir()
    ...
    def test_backup_contents_match_full_backup(self, hermes_home):
        ...
        ``gengar backup`` would, and should exclude the same directories."""
        from hermes_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(hermes_home=hermes_home)
        assert out is not None
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # gengar repo excluded
        assert not any(n.startswith("gengar/") for n in names)
...
class TestRunPreUpdateBackup:
    ...
    def hermes_home(self, tmp_path, monkeypatch):
        ...
        root.mkdir()
        _make_hermes_tree(root)
        # Point GENGAR_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("GENGAR_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for hermes_cli.config + hermes_constants so they pick up GENGAR_HOME
...
# ---------------------------------------------------------------------------
# Pre-migration backup (gengar claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``gengar claw migrate`` runs before mutating ~/.gengar/."""
    ...
    def test_creates_backup_under_backups_dir(self, hermes_home):
        ...
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `gengar
    ...
    def test_backup_uses_shared_exclusion_rules(self, hermes_home):
        ...
        ``gengar backup`` / ``create_pre_update_backup`` — no drift."""
        from hermes_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(hermes_home=hermes_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_restorable_with_hermes_import(self, hermes_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Gengar
        backup — `gengar import` should accept it."""
    ...
    def test_missing_hermes_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.gengar yet — nothing to back up."""
        from hermes_cli.backup import create_pre_migration_backup
