"""Tests for the stale-dashboard handling run at the end of ``gengar update``.

``gengar update`` detects ``gengar dashboard`` processes left over from the
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                ...
                stdout="\n".join([
                    ...
                    _ps_line(12346, "gengar dashboard --port 9120 --no-open"),
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                ...
                stdout="\n".join([
                    ...
                    _ps_line(12345, "gengar dashboard --port 9119"),
                ]) + "\n",
                stderr="",
            )
    ...
    def test_grep_lines_ignored(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(99999, "grep gengar dashboard"),
                    _ps_line(12345, "gengar dashboard --port 9119"),
                ]) + "\n",
                stderr="",
            )
...
class TestKillStaleDashboardPosix:
    ...
    def test_permission_error_is_reported_not_raised(self, capsys):
        ...
        must not abort gengar update — it's reported as a failure and we
        move on."""
        def fake_kill(pid, sig):
...
class TestWindowsWmicEncoding:
    ...
    `gengar update` on non-UTF-8 system locales (e.g. cp936 on zh-CN).
    """
    ...
    def test_wmic_returns_none_stdout_does_not_crash(self, monkeypatch):
        ...
        ``None.split('\\n')`` and aborting `gengar update` (#17049)."""
        monkeypatch.setattr(sys, "platform", "win32")
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
