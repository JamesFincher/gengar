"""Tests for ``gengar dashboard --stop`` / ``--status`` flags.

These flags share the detection + kill path with the post-``gengar update``
...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        ...
        assert "No gengar dashboard processes running" in out
    ...
    def test_status_with_processes(self, capsys):
        ...
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "2 gengar dashboard process(es) running" in out
...
class TestDashboardStop:
    ...
    def test_stop_kills_and_exits_zero_when_all_killed(self, capsys):
        ...
        # for the post-`gengar update` path).
...
class TestLifecycleFlagsTakePrecedence:
    ...
    who typed ``gengar dashboard --stop`` must not end up ALSO starting
    a new server."""
...
class TestArgparseWiring:
    ...
    ``gengar dashboard --stop`` / ``--status`` actually parse."""
