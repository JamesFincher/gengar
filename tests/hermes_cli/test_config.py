...
class TestGetGengarHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("GENGAR_HOME", None)
            home = get_hermes_home()
            assert home == Path.home() / ".gengar"

    def test_env_override(self):
        with patch.dict(os.environ, {"GENGAR_HOME": "/custom/path"}):
            home = get_hermes_home()
            assert home == Path("/custom/path")


class TestEnsureGengarHome:
    def test_creates_subdirs(self, tmp_path):
        with patch.dict(os.environ, {"GENGAR_HOME": str(tmp_path)}):
            ensure_hermes_home()
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"GENGAR_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"GENGAR_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"GENGAR_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"GENGAR_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"GENGAR_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
