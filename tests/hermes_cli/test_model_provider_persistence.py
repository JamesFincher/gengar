"""Tests that provider selection via `gengar model` always persists correctly.
...
def config_home(tmp_path, monkeypatch):
    """Isolated GENGAR_HOME with a minimal string-format config."""
...
class TestBaseUrlValidation:
    ...
    def test_invalid_base_url_rejected(self, config_home, monkeypatch, capsys):
        ...
        with patch("hermes_cli.auth._prompt_model_selection", return_value="glm-5"), \
             patch("hermes_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value="nano ~/.gengar/.env"):
