"""Tests for --ignore-user-config and --ignore-rules flags on `gengar chat`.
...
files. In Gengar the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.gengar/config.yaml`` in ``load_cli_config()``
...
argparse → cmd_chat → cli.main() → GengarCLI → AIAgent call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("GENGAR_IGNORE_USER_CONFIG", "GENGAR_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``GENGAR_IGNORE_USER_CONFIG=1``.
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "anthropic/claude-sonnet-4.6")
        monkeypatch.setenv("GENGAR_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``GengarCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting GENGAR_IGNORE_RULES=1 flips GengarCLI.ignore_rules True."""
        monkeypatch.setenv("GENGAR_IGNORE_RULES", "1")

        # Import GengarCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of GengarCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.GengarCLI)
        # Replicate the exact logic from cli.py GengarCLI.__init__:
        ignore_rules = False  # constructor default
        obj.ignore_rules = ignore_rules or os.environ.get("GENGAR_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True

    def test_constructor_flag_alone_enables_ignore_rules(self, monkeypatch):
        monkeypatch.delenv("GENGAR_IGNORE_RULES", raising=False)
        import cli
        obj = object.__new__(cli.GengarCLI)
...
class TestCmdChatWiring:
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("GENGAR_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("GENGAR_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("GENGAR_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("GENGAR_IGNORE_RULES") == "1"
    ...
    def test_only_ignore_user_config(self, monkeypatch):
        ...
        assert os.environ.get("GENGAR_IGNORE_USER_CONFIG") == "1"
        assert "GENGAR_IGNORE_RULES" not in os.environ
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "GENGAR_IGNORE_USER_CONFIG" not in os.environ
        assert "GENGAR_IGNORE_RULES" not in os.environ
...
class TestArgparseFlagsRegistered:
    ...
    def test_flags_present_in_chat_parser(self):
        ...
        import argparse
        parser = argparse.ArgumentParser(prog="gengar")
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real gengar parser accepts both flags."""
        ...
        assert "GENGAR_IGNORE_USER_CONFIG" in src
        assert "GENGAR_IGNORE_RULES" in src
