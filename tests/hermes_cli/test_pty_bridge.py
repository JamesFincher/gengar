...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf gengar-ok"])
        try:
            output = _read_until(bridge, b"gengar-ok")
            assert b"gengar-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$GENGAR_PTY_TEST\""],
            env={**os.environ, "GENGAR_PTY_TEST": "pty-env-works"},
        )
        try:
