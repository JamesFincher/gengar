...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    # `gengar auth add nous` must also populate providers.nous so the
    ...
    # raises "Gengar is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`gengar auth add nous --type oauth --label <name>` must preserve the
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`gengar logout --provider openai-codex` must still clear model.provider.
...
def test_logout_defaults_to_configured_codex_when_no_active_provider(tmp_path, monkeypatch, capsys):
    """Bare `gengar logout` should target configured Codex if auth has no active provider."""
    hermes_home = tmp_path / "gengar"
    monkeypatch.setenv("GENGAR_HOME", str(hermes_home))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}, "credential_pool": {}})
    (hermes_home / "config.yaml").write_text(
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    """Re-linking codex via `gengar auth add openai-codex` must clear any suppression marker."""
    monkeypatch.setenv("GENGAR_HOME", str(tmp_path / "gengar"))
    monkeypatch.setattr(
        "agent.credential_pool._seed_from_singletons",
        lambda provider, entries: (False, set()),
    ...
    # Pre-existing suppression (simulating a prior `gengar auth remove`)
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    """`gengar auth remove xai 1` must stick even when the env var is exported
    by the shell (not written into ~/.gengar/.env).  Before PR for #13371 the
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in ~/.gengar/.env (not the shell), the
...
def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via `gengar auth add <provider>` clears any
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    via `gengar auth remove`.  This is the gate that prevents shell-exported
...
# =============================================================================
# Unified credential-source stickiness — every source Gengar reads from has a
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via `gengar auth add <provider>` clears ALL
...
def test_auth_remove_codex_manual_device_code_suppresses_canonical(tmp_path, monkeypatch):
    """Removing a manual:device_code entry (from `gengar auth add openai-codex`)
