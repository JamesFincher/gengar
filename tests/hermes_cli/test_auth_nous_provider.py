...
class TestResolveVerifyFallback:
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("GENGAR_CA_BUNDLE", raising=False)
    ...
    def test_missing_hermes_ca_bundle_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.setenv("GENGAR_CA_BUNDLE", "/nonexistent/gengar-ca.pem")
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `gengar model` → Nous Portal → Skip (keep current) after
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `gengar auth add nous`
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "Gengar is not logged into Nous Portal").
    ...
    # doesn't raise "Gengar is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `gengar auth add nous --type oauth --label <name>` silently
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", Gengar must surface an
    ...
    re-authenticate" string led users to report this as a Gengar persistence
    ...
    assert "gengar auth add nous" in message.lower()
