"""Tests for Bug #12905 fix — stale OAuth token detection in gengar model flow.

Bug 3: `gengar model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.gengar/.env but no valid
