"""Tests for Codex auth — tokens stored in Gengar auth store (~/.gengar/auth.json)."""
...
def _setup_hermes_auth(hermes_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Gengar auth store."""
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Gengar auth store, not ~/.codex/."""
    hermes_home = tmp_path / "gengar"
    codex_home = tmp_path / "codex-cli"
    hermes_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "gengar-at", "refresh_token": "gengar-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Gengar store
    assert not (codex_home / "auth.json").exists()

    # Gengar auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "gengar-at"
...
def test_resolve_returns_hermes_auth_store_source(tmp_path, monkeypatch):
    ...
    assert creds["source"] == "gengar-auth-store"
