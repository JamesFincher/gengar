...
- Bare `gengar setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `gengar setup --quick` runs the narrower "fill in missing items" flow.
- `gengar setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
class TestExistingInstallDefault:
    """Bare `gengar setup` on an existing install = full reconfigure wizard."""
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`gengar setup --reconfigure` behaves the same as bare `gengar setup`."""
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["gengar", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["gengar", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["gengar", "setup"])
        try:
            main()
        except SystemExit:
            pass
