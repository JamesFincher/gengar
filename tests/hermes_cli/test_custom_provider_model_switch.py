"""Tests that `gengar model` always shows the model selection menu for custom
...
def config_home(tmp_path, monkeypatch):
    """Isolated GENGAR_HOME with a minimal config."""
...
class TestCustomProviderModelSwitch:
    ...
    def test_key_env_providers_dict_entry_does_not_add_api_key(
        ...
        config_path.write_text(
            ...
            "    key_env: GENGAR_CRS_HENKEE_KEY\n"
            "    transport: anthropic_messages\n"
            "    model: claude-opus-4-7\n"
            "    default_model: claude-opus-4-7\n"
            "custom_providers: []\n"
        )
        monkeypatch.setenv("GENGAR_CRS_HENKEE_KEY", "cr_live_secret_xyz")
        ...
        assert entry["key_env"] == "GENGAR_CRS_HENKEE_KEY"
        ...
        assert "${GENGAR_CRS_HENKEE_KEY}" not in saved_text
    ...
    def test_key_env_providers_dict_preserves_existing_api_key(
        ...
        provider_info = {
            ...
            "api_mode": "anthropic_messages",
            "provider_key": "crs-henkee",
            "api_key_ref": "${GENGAR_CRS_HENKEE_KEY}",  # raw template preserved
        }
        ...
        assert entry["api_key"] == "${GENGAR_CRS_HENKEE_KEY}"
        assert "cr_live_secret_xyz" not in saved_text
