"""Tests for IRC gateway configuration via `gengar setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Gengar install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Gengar install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#gengar")
            monkeypatch.setenv("IRC_NICKNAME", "gengar-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `gengar setup gateway` experience with IRC present."""
