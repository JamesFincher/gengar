...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (GENGAR_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via GENGAR_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in GENGAR_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from hermes_cli.providers import GENGAR_OVERLAYS
        assert "bedrock" in GENGAR_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from hermes_cli.providers import GENGAR_OVERLAYS
        assert GENGAR_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from hermes_cli.providers import GENGAR_OVERLAYS
        assert GENGAR_OVERLAYS["bedrock"].auth_type == "aws_sdk"
