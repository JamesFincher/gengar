"""Tests for the Gengar plugin system (hermes_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    *base* is expected to be ``<hermes_home>/plugins/``; we derive
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <GENGAR_HOME>/config.yaml.
        # Config is always read from GENGAR_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        hermes_home_str = os.environ.get("GENGAR_HOME")
        if hermes_home_str:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.gengar/plugins/ are discovered."""
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("GENGAR_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".gengar" / "plugins"
...
class TestPluginCommands:
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "hermes_test" / "plugins"
        # Set GENGAR_HOME BEFORE _make_plugin_dir so auto-enable targets
