"""Tests for `gengar fallback` — chain reading, add/remove/clear, legacy migration."""
...
# ---------------------------------------------------------------------------
# Shared fixture — isolate GENGAR_HOME so save_config writes to tmp_path
# ---------------------------------------------------------------------------
...
def _write_config(home: Path, data: dict) -> None:
    config_path = home / ".gengar" / "config.yaml"
    ...
    def test_reads_new_list_format(self):
        ...
        cfg = {
            "fallback_providers": [
                ...
                {"provider": "nous", "model": "Gengar-4-Llama-3.1-405B"},
            ]
        }
...
class TestListCommand:
    def test_list_empty(self, isolated_home, capsys):
        ...
        assert "gengar fallback add" in out
...
class TestRemoveCommand:
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        # Picker returns index 1 (the middle entry, "nous / Gengar-4")
...
class TestArgparseWiring:
    """Verify `gengar fallback` is wired into main.py's argparse tree.
