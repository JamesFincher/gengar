...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when GENGAR_HOME has no git repo."""
    import hermes_cli.banner as banner
    ...
    # Point GENGAR_HOME at a temp dir with no gengar/.git
    monkeypatch.setenv("GENGAR_HOME", str(tmp_path))
    with patch("hermes_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    # Build a fake ~/.gengar with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"GENGAR_HOME": str(default_home)}):
