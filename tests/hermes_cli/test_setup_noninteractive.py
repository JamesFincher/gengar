...
class TestNonInteractiveSetup:
    ...
    def test_cmd_setup_defers_no_tty_handling_to_setup_wizard(self):
        """Bare `gengar setup` should reach setup.py, which prints headless guidance."""
        from hermes_cli.main import cmd_setup
    ...
    def test_non_interactive_flag_skips_wizard(self, capsys):
        ...
        with (
            ...
            patch("hermes_cli.setup.get_hermes_home", return_value="/tmp/.gengar"),
        ...
        assert "gengar config set model.provider custom" in out
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        """Bare `gengar` should not prompt for input when no provider exists and stdin is headless."""
    ...
    def test_main_accepts_tts_setup_section(self, monkeypatch):
        """`gengar setup tts` should parse and dispatch like other setup sections."""
        ...
        monkeypatch.setattr("sys.argv", ["gengar", "setup", "tts"])
