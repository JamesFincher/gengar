...
`GENGAR_REDACT_SECRETS` at module-import time. `hermes_cli/main.py` at
...
in .env), the toggle was silently ignored in both `gengar chat` and
`gengar gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml → `GENGAR_REDACT_SECRETS`
...
def test_redact_secrets_default_false_when_unset(tmp_path):
    ...
    `security.redact_secrets: true` explicitly (or GENGAR_REDACT_SECRETS=true).
    """
    hermes_home = tmp_path / ".gengar"
    hermes_home.mkdir()
    (hermes_home / "config.yaml").write_text("{}\n")  # empty config
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env GENGAR_REDACT_SECRETS takes precedence over config.yaml."""
    hermes_home = tmp_path / ".gengar"
    hermes_home.mkdir()
    (hermes_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (hermes_home / ".env").write_text("GENGAR_REDACT_SECRETS=true\n")
