...
Ensures ``gengar update`` correctly detects running gateways managed by
...
def _make_run_side_effect(
    ...
    def side_effect(cmd, **kwargs):
        ...
        # systemctl list-units gengar-gateway* — discover all gateway services
        if "systemctl" in joined and "list-units" in joined:
            if "--user" in joined and systemd_active:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="gengar-gateway.service loaded active running Gengar Gateway\n",
                    stderr="",
                )
...
class TestLaunchdPlistPath:
    def test_plist_contains_environment_variables(self):
        ...
        assert "<key>GENGAR_HOME</key>" in plist
...
class TestCmdUpdateLaunchdRestart:
    ...
    def test_update_detects_launchd_and_skips_manual_restart_message(
        ...
        'auto-restart via launchd' instead of 'Restart it with: gengar gateway run'."""
        ...
        assert "Restart manually: gengar gateway run" not in captured
    ...
    def test_update_without_launchd_shows_manual_restart(
        ...
        captured = capsys.readouterr().out
        assert "Restart manually: gengar gateway run" in captured
    ...
    def test_update_restarts_profile_manual_gateways(
        ...
        monkeypatch.setattr(
            ...
            lambda: tmp_path / "ai.gengar.gateway.plist",
        )
        ...
        process = gateway_cli.ProfileGatewayProcess(
            profile="coder",
            path=tmp_path / ".gengar" / "profiles" / "coder",
            pid=12345,
        )
    ...
    def test_update_with_systemd_still_restarts_via_systemd(
        ...
        captured = capsys.readouterr().out
        assert "Restarted gengar-gateway" in captured
        ...
        def side_effect(cmd, **kwargs):
            ...
            if "systemctl" in joined and "list-units" in joined:
                if "--user" in joined:
                    return subprocess.CompletedProcess(
                        cmd, 0,
                        stdout="gengar-gateway.service loaded active running\n",
                        stderr="",
                    )
                return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
...
class TestServicePidExclusion:
    ...
    where ``gengar update`` would restart the gateway and immediately kill it.
    """
...
class TestGetServicePids:
    ...
    def test_returns_launchd_pid(self, monkeypatch):
        monkeypatch.setattr(gateway_cli, "is_linux", lambda: False)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: True)
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.gengar.gateway")
    ...
    def test_excludes_zero_pid(self, monkeypatch):
        ...
        def fake_run(cmd, **kwargs):
            ...
            if "list-units" in joined:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="gengar-gateway.service loaded inactive dead Gengar Gateway\n",
                    stderr="",
                )
            if "show" in joined and "MainPID" in joined:
    ...
    def test_filters_to_current_profile(self, monkeypatch, tmp_path):
        ...
        def fake_run(cmd, **kwargs):
            return subprocess.CompletedProcess(
                cmd, 0,
                stdout=(
                    "100 /Users/dgrieco/.gengar/gengar/venv/bin/python -m hermes_cli.main --profile orcha gateway run --replace\n"
                    "200 /Users/dgrieco/.gengar/gengar/venv/bin/python -m hermes_cli.main --profile other gateway run --replace\n"
                ),
                stderr="",
            )
...
class TestGatewayModeWritesExitCodeEarly:
    """When running as ``gengar update --gateway``, the exit code marker must be
    ...
    def test_exit_code_written_in_gateway_mode(
        ...
        # Point GENGAR_HOME at a temp dir so the marker file lands there
...
class TestCmdUpdateLegacyGatewayWarning:
    """Tests for the legacy gengar.service warning printed by `gengar update`.

    Users who installed Gengar before the service rename often have a
    dormant ``gengar.service`` that starts flap-fighting the current
    ``gengar-gateway.service`` after PR #5646. Every ``gengar update``
    should remind them to run ``gengar gateway migrate-legacy`` until
    they do.
    """
    ...
    def test_update_prints_legacy_warning_when_detected(
        ...
        user_dir.mkdir()
        system_dir.mkdir()
        legacy_path = user_dir / "gengar.service"
        ...
        captured = capsys.readouterr().out
        assert "Legacy Gengar gateway unit(s) detected" in captured
        assert "gengar.service" in captured
        assert "gengar gateway migrate-legacy" in captured
    ...
    def test_update_silent_when_no_legacy_units(
        ...
        captured = capsys.readouterr().out
        assert "Legacy Gengar gateway" not in captured
    ...
    def test_update_does_not_flag_profile_units(
        ...
        """Profile units (gengar-gateway-coder.service) must not trigger the warning.

        This is the core safety invariant: the legacy allowlist is
        ``gengar.service`` only, no globs.
        """
        user_dir = tmp_path / "user"
        system_dir = tmp_path / "system"
        user_dir.mkdir()
        ...
        (user_dir / "gengar-gateway-coder.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
        (user_dir / "gengar-gateway.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
        ...
        captured = capsys.readouterr().out
        assert "Legacy Gengar gateway" not in captured
        assert "gengar-gateway-coder.service" not in captured  # not flagged
    ...
    def test_update_lists_system_scope_unit_with_sudo_hint(
        ...
        captured = capsys.readouterr().out
        assert "Legacy Gengar gateway" in captured
