...
def test_do_list_platform_env_is_ignored(three_source_env, monkeypatch):
    """`gengar skills list` reads the active profile's config via
    GENGAR_HOME (swapped by -p), so it must NOT pass a platform arg to
    ``get_disabled_skill_names`` — otherwise per-platform overrides
    would silently leak in from GENGAR_PLATFORM env."""
    from agent import skill_utils
