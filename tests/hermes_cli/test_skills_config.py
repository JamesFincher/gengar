...
class TestIsSkillDisabled:
    ...
    @patch("hermes_cli.config.load_config")
    @patch.dict("os.environ", {"GENGAR_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_explicit_platform_param(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("GENGAR_PLATFORM", raising=False)
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """GENGAR_SESSION_PLATFORM should be used when GENGAR_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_hermes_platform_takes_precedence(self, tmp_path, monkeypatch):
        """GENGAR_PLATFORM should win over GENGAR_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
        ...
        monkeypatch.setenv("GENGAR_HOME", str(tmp_path))
        monkeypatch.setenv("GENGAR_PLATFORM", "telegram")
        monkeypatch.setenv("GENGAR_SESSION_PLATFORM", "discord")
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("GENGAR_HOME", str(tmp_path))
        monkeypatch.setenv("GENGAR_PLATFORM", "telegram")
        monkeypatch.setenv("GENGAR_SESSION_PLATFORM", "telegram")
