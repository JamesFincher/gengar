...
PROVIDER_ENV_VARS = (
    ...
    "OPENAI_BASE_URL", "GENGAR_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH",
    "GENGAR_COPILOT_ACP_ARGS", "COPILOT_ACP_BASE_URL",
)
...
class TestApiKeyProviderStatus:
    ...
    def test_copilot_acp_status_detects_local_cli(self, monkeypatch):
        monkeypatch.setenv("GENGAR_COPILOT_ACP_ARGS", "--acp --stdio --debug")
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_resolve_copilot_acp_with_local_cli(self, monkeypatch):
        monkeypatch.setenv("GENGAR_COPILOT_ACP_ARGS", "--acp --stdio")
        monkeypatch.setattr("hermes_cli.auth.shutil.which", lambda command: f"/usr/local/bin/{command}")
...
class TestHasAnyProviderConfigured:
    ...
    def test_claude_code_creds_ignored_on_fresh_install(self, monkeypatch, tmp_path):
        """Claude Code credentials should NOT skip the wizard when Gengar is unconfigured."""
        from hermes_cli import config as config_module
    ...
    def test_claude_code_creds_counted_when_hermes_configured(self, monkeypatch, tmp_path):
        """Claude Code credentials should count when Gengar has been explicitly configured."""
        import yaml
        from hermes_cli import config as config_module
        hermes_home = tmp_path / ".gengar"
        hermes_home.mkdir()
