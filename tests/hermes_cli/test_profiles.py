...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and GENGAR_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.gengar/profiles)
    * GENGAR_HOME  -> tmp_path/.gengar  (so get_hermes_home() agrees)
    * Creates the bare-minimum ~/.gengar directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".gengar"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("GENGAR_HOME", str(default_home))
    return tmp_path
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.gengar must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".gengar"
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".gengar" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_hermes_home_returns_default(self, profile_env):
        # GENGAR_HOME points to tmp_path/.gengar which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".gengar" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom GENGAR_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("GENGAR_HOME", str(custom))
        # With Docker-aware roots, a custom GENGAR_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("gengar")
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".gengar" / "profiles" / "oldname"
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "gengar.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["gengar.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["gengar.heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "gengar.ssi_health": {"workspace": "gengar", "enabled": True}
            }
        }))
        ...
        assert "gengar.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["gengar.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["gengar.heimdall"]["workspace"] == "gengar"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "gengar.ssi_health": {"aiPeer": "ssi_health"},
                "gengar.heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["gengar.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["gengar.heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("gengar", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".gengar_history",
        ...
        excluded_prefixes = [
            "default/gengar", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.gengar_history",
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultGengarHome (internal helpers)
# ===================================================================
...
class TestInternalHelpers:
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (GENGAR_HOME under ~/.gengar), profiles root is still ~/.gengar/profiles."""
        native = tmp_path / ".gengar"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under GENGAR_HOME."""
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under GENGAR_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("GENGAR_HOME", str(docker_home))
    ...
    def test_active_profile_name_docker_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("GENGAR_HOME", str(profile))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".gengar" / "profiles" / "mybot"
        assert result == expected
