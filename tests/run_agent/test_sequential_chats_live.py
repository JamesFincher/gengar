...
AlexKucera reported on Discord (2026-04-16) that after ``gengar update`` pulled
...
Opt-in — not part of default CI:
    GENGAR_LIVE_TESTS=1 pytest tests/run_agent/test_sequential_chats_live.py -v

Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.gengar/.env).
"""
...
LIVE = os.environ.get("GENGAR_LIVE_TESTS") == "1"
OR_KEY = os.environ.get("OPENROUTER_API_KEY", "")

pytestmark = [
    pytest.mark.skipif(not LIVE, reason="live-only — set GENGAR_LIVE_TESTS=1"),
