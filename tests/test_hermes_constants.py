...
class TestGetDefaultGengarRoot:
    """Tests for get_default_hermes_root() — Docker/custom deployment awareness."""

    def test_no_hermes_home_returns_native(self, tmp_path, monkeypatch):
        """When GENGAR_HOME is not set, returns ~/.gengar."""
        monkeypatch.delenv("GENGAR_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        assert get_default_hermes_root() == tmp_path / ".gengar"

    def test_hermes_home_is_native(self, tmp_path, monkeypatch):
        """When GENGAR_HOME = ~/.gengar, returns ~/.gengar."""
        native = tmp_path / ".gengar"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("GENGAR_HOME", str(native))
        assert get_default_hermes_root() == native

    def test_hermes_home_is_profile(self, tmp_path, monkeypatch):
        """When GENGAR_HOME is a profile under ~/.gengar, returns ~/.gengar."""
        native = tmp_path / ".gengar"
    ...
    def test_hermes_home_is_docker(self, tmp_path, monkeypatch):
        """When GENGAR_HOME points outside ~/.gengar (Docker), returns GENGAR_HOME."""
    ...
    def test_hermes_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any GENGAR_HOME outside ~/.gengar is treated as the root."""
        custom = tmp_path / "my-gengar-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (GENGAR_HOME=<root>/profiles/<name>),
