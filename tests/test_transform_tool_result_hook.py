...
def test_result_unchanged_when_no_hook_registered(monkeypatch):
    ...
    monkeypatch.setenv("GENGAR_HOME", "/tmp/hermes_no_plugins")
...
def test_transform_tool_result_integration_with_real_plugin(monkeypatch, tmp_path):
    """End-to-end: load a real plugin from GENGAR_HOME and verify it rewrites results."""
    import yaml

    hermes_home = Path(os.environ["GENGAR_HOME"])
