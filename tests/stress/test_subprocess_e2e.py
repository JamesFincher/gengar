...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - worker logs are captured to GENGAR_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "GENGAR_KANBAN_TASK": task.id,
            "GENGAR_KANBAN_WORKSPACE": workspace,
...
def main():
    ...
    # Point the `gengar` CLI child processes will run at the worktree
    # hermes_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "gengar")
