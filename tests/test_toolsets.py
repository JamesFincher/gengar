...
class TestToolsetConsistency:
    ...
    def test_hermes_platforms_share_core_tools(self):
        """All gengar-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on gengar-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["gengar-cli", "gengar-telegram", "gengar-discord", "gengar-whatsapp", "gengar-slack", "gengar-signal", "gengar-homeassistant"]
