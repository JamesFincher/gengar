...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "gengar"
        assert config.workspace_id == "gengar"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.gengar/honcho.json
        monkeypatch.setenv("GENGAR_HOME", str(tmp_path / "isolated"))
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"gengar": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"gengar": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"gengar": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="gengar"
        ):
            result = config.resolve_session_name("/home/user/gengar/subdir")
...
class TestResolveConfigPath:
    def test_prefers_hermes_home_when_exists(self, tmp_path):
        ...
        with patch.dict(os.environ, {"GENGAR_HOME": str(hermes_home)}):
    ...
    def test_falls_back_to_global_when_no_local(self, tmp_path):
        hermes_home = tmp_path / "gengar"
        hermes_home.mkdir()
        # No honcho.json in GENGAR_HOME — also isolate ~/.gengar so
    ...
    def test_falls_back_to_global_without_hermes_home_env(self, tmp_path):
        ...
        with patch.dict(os.environ, {}, clear=False), \
             patch.object(Path, "home", return_value=fake_home):
            os.environ.pop("GENGAR_HOME", None)
            result = resolve_config_path()
        assert result == fake_home / ".honcho" / "config.json"
...
class TestResolveActiveHost:
    def test_default_returns_hermes(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("GENGAR_HONCHO_HOST", None)
            os.environ.pop("GENGAR_HOME", None)
            assert resolve_active_host() == "gengar"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"GENGAR_HONCHO_HOST": "gengar.coder"}):
            assert resolve_active_host() == "gengar.coder"
...
class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="gengar.coder")
        assert config.host == "gengar.coder"
        assert config.workspace_id == "gengar"  # shared workspace
        assert config.ai_peer == "gengar.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="gengar")
        assert config.host == "gengar"
        assert config.workspace_id == "gengar"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "gengar": {"aiPeer": "gengar", "peerName": "alice"},
                "gengar.coder": {
                    "aiPeer": "gengar.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="gengar.coder", config_path=config_file,
        )
        assert config.host == "gengar.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "gengar.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="gengar.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "gengar.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"gengar": {"enabled": True, "aiPeer": "gengar"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"gengar": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
