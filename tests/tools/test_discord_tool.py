...
# ---------------------------------------------------------------------------
# Toolset: discord / discord_admin only in gengar-discord
# ---------------------------------------------------------------------------

class TestToolsetInclusion:
    def test_discord_tools_in_hermes_discord_toolset(self):
        from toolsets import TOOLSETS
        assert "discord" in TOOLSETS["gengar-discord"]["tools"]
        assert "discord_admin" in TOOLSETS["gengar-discord"]["tools"]

    def test_discord_tools_not_in_core_tools(self):
        from toolsets import _GENGAR_CORE_TOOLS
        assert "discord" not in _GENGAR_CORE_TOOLS
        assert "discord_admin" not in _GENGAR_CORE_TOOLS

    def test_discord_tools_not_in_other_toolsets(self):
        from toolsets import TOOLSETS
        for name, ts in TOOLSETS.items():
            if name in ("gengar-discord", "gengar-gateway", "discord", "discord_admin"):
                continue
            tools = ts.get("tools", [])
...
class TestModelToolsIntegration:
    ...
    def test_discord_admin_schema_rebuilt_by_get_tool_definitions(
        ...
        tools = get_tool_definitions(enabled_toolsets=["gengar-discord"], quiet_mode=True)
