...
``~/.gengar/.env`` entries. These tests confirm each STT provider now
...
class TestProviderSelectionGate:
    ...
    ``os.environ`` a user with keys in ``~/.gengar/.env`` would be told
...
class TestEndToEndRegressionGuard:
    ...
    ``~/.gengar/.env`` carrying the key while ``os.environ`` does not.
