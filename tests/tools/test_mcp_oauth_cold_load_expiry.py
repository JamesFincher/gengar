...
1. ``GengarTokenStorage.set_tokens`` persisted only relative ``expires_in``,
...
- ``GengarMCPOAuthProvider._initialize`` seeds ``context.token_expiry_time``
...
# ---------------------------------------------------------------------------
# GengarTokenStorage — absolute expiry persistence
# ---------------------------------------------------------------------------
...
class TestSetTokensAbsoluteExpiry:
    def test_set_tokens_persists_absolute_expires_at(self, tmp_path, monkeypatch):
        ...
        from tools.mcp_oauth import GengarTokenStorage

        storage = GengarTokenStorage("srv")
        before = time.time()
        asyncio.run(
            storage.set_tokens(
                OAuthToken(
...
class TestGetTokensReconstructsExpiresIn:
    ...
    def test_get_tokens_returns_zero_ttl_for_expired_token(
        ...
        monkeypatch.setenv("GENGAR_HOME", str(tmp_path))
        from tools.mcp_oauth import GengarTokenStorage, _get_token_dir
...
# ---------------------------------------------------------------------------
# GengarMCPOAuthProvider._initialize — seed token_expiry_time
# ---------------------------------------------------------------------------
...
async def test_initialize_seeds_token_expiry_time_from_stored_tokens(
    ...
    from tools.mcp_oauth import GengarTokenStorage
    from tools.mcp_oauth_manager import _GENGAR_PROVIDER_CLS, reset_manager_for_tests

    assert _GENGAR_PROVIDER_CLS is not None
...
async def test_initialize_skips_prefetch_when_no_tokens(tmp_path, monkeypatch):
    ...
    storage = GengarTokenStorage("srv")  # empty — no tokens on disk
    metadata = OAuthClientMetadata(
        redirect_uris=[AnyUrl("http://127.0.0.1:12345/callback")],
        client_name="Gengar",
    )
