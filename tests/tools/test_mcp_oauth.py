...
class TestGengarTokenStorage:
    def test_roundtrip_tokens(self, tmp_path, monkeypatch):
        monkeypatch.setenv("GENGAR_HOME", str(tmp_path))
        storage = GengarTokenStorage("test-server")
    ...
    def test_has_cached_tokens(self, tmp_path, monkeypatch):
        monkeypatch.setenv("GENGAR_HOME", str(tmp_path))
        storage = GengarTokenStorage("my-server")
    ...
    def test_corrupt_tokens_returns_none(self, tmp_path, monkeypatch):
        monkeypatch.setenv("GENGAR_HOME", str(tmp_path))
        storage = GengarTokenStorage("bad-server")
...
class TestPathTraversal:
    ...
    def test_path_traversal_blocked(self, tmp_path, monkeypatch):
        monkeypatch.setenv("GENGAR_HOME", str(tmp_path))
        storage = GengarTokenStorage("../../.ssh/config")
    ...
    def test_dots_and_slashes_sanitized(self, tmp_path, monkeypatch):
        monkeypatch.setenv("GENGAR_HOME", str(tmp_path))
        storage = GengarTokenStorage("../../../etc/passwd")
        path = storage._tokens_path()
...
def test_build_oauth_auth_preserves_server_url_path():
    ...
    ``OAuthContext.get_authorization_base_url``; Gengar must not pre-strip.
    """
    ...
    with patch.object(mcp_oauth, "_OAUTH_AVAILABLE", True), \
         ...
         patch.object(mcp_oauth, "GengarTokenStorage") as mock_storage_cls:
