...
class TestFirecrawlClientConfig:
    ...
    def test_tool_gateway_domain_builds_firecrawl_gateway_origin(self):
        ...
        with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "jamesfincher.com"}):
            with patch("tools.web_tools._read_nous_access_token", return_value="nous-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="nous-token",
                        api_url="https://firecrawl-gateway.jamesfincher.com",
                    )
    ...
    def test_tool_gateway_scheme_can_switch_derived_gateway_origin_to_http(self):
        ...
        with patch.dict(os.environ, {
            "TOOL_GATEWAY_DOMAIN": "jamesfincher.com",
            ...
            with patch("tools.web_tools._read_nous_access_token", return_value="nous-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="nous-token",
                        api_url="http://firecrawl-gateway.jamesfincher.com",
                    )
                    assert result is mock_fc.return_value
    ...
    def test_nous_auth_token_respects_hermes_home_override(self, tmp_path):
        """Auth lookup should read from GENGAR_HOME/auth.json, not ~/.gengar/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".gengar").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``gengar tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
