...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.gengar/token.json"
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.gengar/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        ...
        with patch.dict(os.environ, {"GENGAR_HOME": str(hermes_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.gengar")

        assert mounts[0]["container_path"] == "/home/user/.gengar/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.gengar/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.gengar/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape GENGAR_HOME."""
        monkeypatch.setenv("GENGAR_HOME", str(tmp_path / ".gengar"))
        (tmp_path / ".gengar").mkdir()
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside GENGAR_HOME must still be registered."""
        hermes_home = tmp_path / ".gengar"
        hermes_home.mkdir()
        monkeypatch.setenv("GENGAR_HOME", str(hermes_home))
    ...
    def test_nested_subdir_inside_hermes_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of GENGAR_HOME must be allowed."""
        hermes_home = tmp_path / ".gengar"
        hermes_home.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside GENGAR_HOME pointing outside must be rejected."""
        hermes_home = tmp_path / ".gengar"
        hermes_home.mkdir()
        monkeypatch.setenv("GENGAR_HOME", str(hermes_home))
        ...
        # The resolved path escapes GENGAR_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape GENGAR_HOME."""
        hermes_home = tmp_path / ".gengar"
        hermes_home.mkdir()
        monkeypatch.setenv("GENGAR_HOME", str(hermes_home))
    ...
    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside GENGAR_HOME via config must still mount."""
        hermes_home = tmp_path / ".gengar"
        hermes_home.mkdir()
        monkeypatch.setenv("GENGAR_HOME", str(hermes_home))
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        assert "/root/.gengar/cache/documents" in paths
        assert "/root/.gengar/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.gengar/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        assert "/root/.gengar/cache/documents" in container_paths
        assert "/root/.gengar/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        assert entries[0]["container_path"] == "/root/.gengar/cache/screenshots/session_abc/screen1.png"
