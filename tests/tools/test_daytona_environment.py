...
class TestPersistence:
    def test_persistent_resumes_via_get(self, make_env):
        ...
        env._mock_client.get.assert_called_once_with("gengar-mytask")
    ...
    def test_persistent_creates_new_when_none_found(self, make_env, daytona_sdk):
        ...
        env._mock_client.get.assert_called_with("gengar-mytask")
...
class TestExecute:
    ...
    def test_stdin_data_wraps_heredoc(self, make_env):
        ...
        # Base class uses GENGAR_STDIN_ prefix for heredoc delimiters
        call_args = sb.process.exec.call_args_list[-1]
        cmd = call_args[0][0]
        assert "GENGAR_STDIN_" in cmd
        assert "print" in cmd
        assert "hi" in cmd
