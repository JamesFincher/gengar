...
class TestModalBulkUpload:
    ...
    def test_tar_archive_contains_all_files(self, monkeypatch, tmp_path):
        ...
        files = [
            (str(src_a), "/root/.gengar/credentials/a.json"),
            (str(src_b), "/root/.gengar/skills/b.py"),
        ]
        ...
        with tarfile.open(fileobj=buf, mode="r:gz") as tar:
            ...
            assert "root/.gengar/credentials/a.json" in names
            assert "root/.gengar/skills/b.py" in names

            # Verify content
            a_content = tar.extractfile("root/.gengar/credentials/a.json").read()
            assert a_content == b"cred_content"
            b_content = tar.extractfile("root/.gengar/skills/b.py").read()
    ...
    def test_mkdir_includes_all_parents(self, monkeypatch, tmp_path):
        ...
        files = [
            (str(src), "/root/.gengar/credentials/f.txt"),
            (str(src), "/root/.gengar/skills/deep/nested/f.txt"),
        ]
        ...
        cmd = exec_calls[0][2]
        assert "/root/.gengar/credentials" in cmd
        assert "/root/.gengar/skills/deep/nested" in cmd
    ...
    def test_single_exec_call(self, monkeypatch, tmp_path):
        ...
        for i in range(20):
            ...
            files.append((str(src), f"/root/.gengar/cache/file_{i}.txt"))
    ...
    def test_bulk_upload_wired_in_filesyncmanager(self, monkeypatch):
        ...
        env._sync_manager = modal_env.FileSyncManager(
            get_files_fn=lambda: iter_sync_files("/root/.gengar"),
    ...
    def test_timeout_set_to_120(self, monkeypatch, tmp_path):
        ...
        src = tmp_path / "f.txt"
        src.write_text("data")
        files = [(str(src), "/root/.gengar/f.txt")]
    ...
    def test_stdin_chunked_for_large_payloads(self, monkeypatch, tmp_path):
        ...
        files = [(str(src), "/root/.gengar/large.bin")]
        ...
        with tarfile.open(fileobj=buf, mode="r:gz") as tar:
            names = tar.getnames()
            assert "root/.gengar/large.bin" in names
