...
def isolated_session(monkeypatch):
    ...
    monkeypatch.setenv("GENGAR_SESSION_KEY", session_key)
...
class TestCliPathFiresHooks:
    """CLI-interactive approval path: GENGAR_INTERACTIVE is set, the
...
class TestGatewayPathFiresHooks:
    """Async gateway approval path: GENGAR_GATEWAY_SESSION is set and a
    ...
    def test_pre_and_post_fire_on_gateway_surface(
        ...
        monkeypatch.setenv("GENGAR_GATEWAY_SESSION", "1")
        monkeypatch.delenv("GENGAR_EXEC_ASK", raising=False)
        monkeypatch.setattr(approval_module, "_get_approval_mode", lambda: "manual")
