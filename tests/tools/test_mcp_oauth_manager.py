...
def test_hermes_provider_subclass_exists():
    """GengarMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _GENGAR_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _GENGAR_PROVIDER_CLS is not None
    assert issubclass(_GENGAR_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_hermes_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns GengarMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _GENGAR_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("GENGAR_HOME", str(tmp_path))
    ...
    assert _GENGAR_PROVIDER_CLS is not None
    assert isinstance(provider, _GENGAR_PROVIDER_CLS)
