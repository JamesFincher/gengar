...
class TestWriteDenyExactPaths:
    ...
    def test_hermes_env(self):
        # ``.env`` under the active GENGAR_HOME (profile-aware, not just
        # ``~/.gengar``) must be write-denied. The hermetic test conftest
        # points GENGAR_HOME at a tempdir — resolve via get_hermes_home()
...
class TestWriteAllowed:
    ...
    def test_hermes_config_not_env(self):
        path = os.path.join(str(Path.home()), ".gengar", "config.yaml")
