...
- _seed_from_env reads API keys from ~/.gengar/.env when not in os.environ
...
def isolated_hermes_home(tmp_path, monkeypatch):
    """Point GENGAR_HOME at a temp dir and clear known API key env vars.
...
def _write_env_file(home: Path, **kwargs) -> None:
    """Write key=value pairs to ~/.gengar/.env."""
    lines = [f"{k}={v}" for k, v in kwargs.items()]
    (home / ".env").write_text("\n".join(lines) + "\n")


class TestCredentialPoolSeedsFromDotEnv:
    """_seed_from_env must read keys from ~/.gengar/.env, not just os.environ.
...
class TestAuthResolvesFromDotEnv:
    """_resolve_api_key_provider_secret must also read from ~/.gengar/.env."""
