...
dicts) keeps the test independent of the user's ~/.gengar/config.yaml and
...
def _save_config_env_sync_keys() -> set[str]:
    """terminal config keys bridged by ``gengar config set foo bar``."""
...
def test_save_config_set_supports_critical_bridged_keys():
    """``gengar config set terminal.X true`` must propagate to .env for
    ...
    assert not missing, (
        f"`gengar config set terminal.X` doesn't sync these load-bearing "
