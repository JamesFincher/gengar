...
def _clean_state():
    ...
    saved = {}
    for k in ("GENGAR_INTERACTIVE", "GENGAR_GATEWAY_SESSION", "GENGAR_EXEC_ASK", "GENGAR_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["GENGAR_INTERACTIVE"] = "1"
        session_key = os.getenv("GENGAR_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No GENGAR_INTERACTIVE or GENGAR_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
