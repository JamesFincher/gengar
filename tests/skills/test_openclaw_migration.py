...
"""
    ...
    assert "Tyler Williams > Active Projects: Gengar" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `gengar cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    #                      tell the user to *review* ~/.gengar/config.yaml;
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Gengar``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Gengar prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Gengar to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Gengar config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Gengar uses tools well"
    # All-lowercase matches → lowercase ``gengar``; this preserves the
    # real filesystem path ``~/.gengar`` (Gengar home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "gengar should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Gengar remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "gengar prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Gengar was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "gengar uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Gengar said to ask Gengar about Gengar settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.gengar`` (the real Gengar home), not the broken
    ``.Gengar``.
    ...
    rewritten to ``~/.Gengar/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.gengar/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .gengar directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.gengar'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "gengar config path: ~/.gengar/"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are Gengar" in result
