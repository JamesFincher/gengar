"""Tests for the bundled gengar-achievements dashboard plugin.
...
``plugins/gengar-achievements/tests/`` covering the achievement engine
internals (tier math, secret-state handling, catalog invariants). These
tests live at the gengar level and focus on the integration
...
def plugin_api(tmp_path, monkeypatch):
    """Load plugin_api with isolated ~/.gengar so state/snapshot files don't collide.
