...
def test_normalize_retain_tags_accepts_csv_and_dedupes():
    assert _normalize_retain_tags("agent:fakeassistantname, source_system:gengar, agent:fakeassistantname") == [
        "agent:fakeassistantname",
        "source_system:gengar",
    ]


def test_normalize_retain_tags_accepts_json_array_string():
    value = json.dumps(["agent:fakeassistantname", "source_system:gengar"])
    assert _normalize_retain_tags(value) == ["agent:fakeassistantname", "source_system:gengar"]
...
class TestConfig:
    def test_default_values(self, provider):
        ...
        assert provider._retain_context == "conversation between Gengar and the User"

    def test_custom_config_values(self, provider_with_config):
        p = provider_with_config(
            retain_tags=["tag1", "tag2"],
            retain_source="gengar",
        ...
        assert p._retain_source == "gengar"
    ...
    def test_config_from_env_fallback(self, tmp_path, monkeypatch):
        ...
        assert cfg["banks"]["gengar"]["bankId"] == "env-bank"
        assert cfg["banks"]["gengar"]["budget"] == "high"
...
class TestPostSetup:
    def test_local_embedded_setup_materializes_profile_env(self, tmp_path, monkeypatch):
        hermes_home = tmp_path / "gengar-home"
        ...
        profile_env = user_home / ".hindsight" / "profiles" / "gengar.env"
    ...
    def test_local_embedded_setup_respects_existing_profile_name(self, tmp_path, monkeypatch):
        ...
        hermes_env = user_home / ".hindsight" / "profiles" / "gengar.env"
...
class TestSyncTurn:
    def test_sync_turn_retains_metadata_rich_turn(self, provider_with_config):
        ...
        assert item["context"] == "conversation between Gengar and the User"
        ...
        assert item["metadata"]["source"] == "gengar"
    ...
    def test_sync_turn_uses_aretain_batch(self, provider):
        ...
        assert call_kwargs["retain_async"] is True
        assert len(call_kwargs["items"]) == 1
        assert call_kwargs["items"][0]["context"] == "conversation between Gengar and the User"
...
class TestBankIdTemplate:
    def test_sanitize_bank_segment_passthrough(self):
        assert _sanitize_bank_segment("gengar") == "gengar"
    ...
    def test_resolve_collapses_empty_placeholders(self):
        # When user is empty, "gengar-{user}" becomes "gengar-" -> trimmed to "gengar"
        result = _resolve_bank_id_template(
            "gengar-{user}", fallback="default",
    ...
    def test_resolve_invalid_template_returns_fallback(self):
        ...
        result = _resolve_bank_id_template(
            "gengar-{unknown}", fallback="gengar",
            profile="", workspace="", platform="", user="", session="",
        )
        assert result == "gengar"
    ...
    def test_provider_uses_bank_id_template_from_config(self, tmp_path, monkeypatch):
        config = {
            ...
            "bank_id_template": "gengar-{profile}",
        }
        config_path = tmp_path / "hindsight" / "config.json"
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        ...
        p.initialize(
            ...
            agent_workspace="gengar",
        )
        assert p._bank_id == "gengar-coder"
        assert p._bank_id_template == "gengar-{profile}"
    ...
    def test_provider_template_with_missing_profile_falls_back(self, tmp_path, monkeypatch):
        ...
        p = HindsightMemoryProvider()
        # No agent_identity passed — template renders to "gengar-" which collapses to "gengar"
        p.initialize(session_id="s1", hermes_home=str(tmp_path), platform="cli")
        assert p._bank_id == "gengar"
