...
def test_registry_defaults_to_hermes_home(tmp_path, monkeypatch):
    ...
    # _isolate_home already set GENGAR_HOME to tmp_path/.gengar; the
    # registry default path must live inside that tree.
    r = NodeRegistry()
    r.add("x", "ws://x", "t")
    expected = Path(tmp_path) / ".gengar" / "workspace" / "meetings" / "nodes.json"
...
def test_cli_status_pings_via_node_client(capsys, monkeypatch):
    ...
    class _FakeClient:
        ...
        def ping(self):
            return {"type": "pong", "display_name": "gengar-meet-node"}
