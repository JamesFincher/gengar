...
class _SafeWriter:
    ...
    When gengar runs as a systemd service, Docker container, or headless
...
def _routermint_headers() -> dict:
    ...
    from hermes_cli import __version__ as _GENGAR_VERSION

    return {
        "User-Agent": f"Gengar/{_GENGAR_VERSION}",
    }
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[gengar: tool call arguments were corrupted in this session and "
    ...
    def __init__(
        ...
        Args:
            ...
            load_soul_identity (bool): If True, still use ~/.gengar/SOUL.md as the primary
        ...
        # both live under ~/.gengar/logs/.  Idempotent, so gateway mode
                    ...
                    if _explicit and _explicit not in ("auto", "openrouter", "custom"):
                        ...
                        if not _fb_resolved:
                            raise RuntimeError(
                                ...
                                f"variable, or switch to a different provider with `gengar model`."
                            )
                    if not getattr(self, "_fallback_activated", False):
                        # No provider configured — reject with a clear message.
                        raise RuntimeError(
                            "No LLM provider configured. Run `gengar model` to "
                            "select a provider, or run `gengar setup` for first-time "
                            "configuration."
                        )
        ...
        # Session logs go into ~/.gengar/sessions/ alongside gateway sessions
                ...
                if _mem_provider_name:
                    ...
                    if self._memory_manager.providers:
                        ...
                        try:
                            ...
                            _init_kwargs["agent_workspace"] = "gengar"
                        except Exception:
                            pass
        ...
        if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"by Gengar.  Choose a model with at least "
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            self._session_db.create_session(
                session_id=self.session_id,
                source=self.platform or os.environ.get("GENGAR_SESSION_SOURCE", "cli"),
                model=self.model,
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """
        Preload the LM Studio model with at least Gengar' minimum context.
        """
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``gengar chat -q``. In that mode,
        ...
        try:
            ...
            if client is None or not aux_model:
                msg = (
                    ...
                    "Run `gengar setup` or set OPENROUTER_API_KEY."
                )
            ...
            if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
                raise ValueError(
                    ...
                    f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Gengar "
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``GENGAR_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``GENGAR_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return float(os.getenv("GENGAR_API_TIMEOUT", 1800.0))
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``GENGAR_API_CALL_STALE_TIMEOUT`` env var
          4. 300.0s default
        ...
        env_timeout = os.getenv("GENGAR_API_CALL_STALE_TIMEOUT")
    ...
    def _build_memory_write_metadata(
        ...
        *,
        ...
        metadata: Dict[str, Any] = {
            ...
            "platform": self.platform or os.environ.get("GENGAR_SESSION_SOURCE", "cli"),
            "tool_name": "memory",
        }
        if task_id:
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        try:
            ...
            if env_var_enabled("GENGAR_DUMP_REQUEST_STDOUT"):
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        # Some execution modes (cron) still want GENGAR_HOME persona while keeping
        ...
        # Pointer to the gengar skill + docs for user questions about Gengar itself.
        prompt_parts.append(GENGAR_AGENT_HELP_GUIDANCE)
        ...
        # GENGAR_KANBAN_TASK env var). Normal chat sessions never see
        # this block.
        ...
        if not self.skip_context_files:
            ...
            # mode).  The gateway process runs from the gengar install
    ...
    def _try_refresh_nous_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            ...
            creds = resolve_nous_runtime_credentials(
                min_key_ttl_seconds=max(60, int(os.getenv("GENGAR_NOUS_MIN_KEY_TTL_SECONDS", "1800"))),
                timeout_seconds=float(os.getenv("GENGAR_NOUS_TIMEOUT_SECONDS", "15")),
                force_mint=force,
            )
        except Exception as exc:
    ...
    def _interruptible_streaming_api_call(
        ...
        def _call_chat_completions():
            ...
            # wins over the GENGAR_API_TIMEOUT env default if the user set it.
            _provider_timeout_cfg = get_provider_request_timeout(self.provider, self.model)
            _base_timeout = (
                _provider_timeout_cfg
                if _provider_timeout_cfg is not None
                else float(os.getenv("GENGAR_API_TIMEOUT", 1800.0))
            )
            # Read timeout: config wins here too.  Otherwise use
            # GENGAR_STREAM_READ_TIMEOUT (default 120s) for cloud providers.
            if _provider_timeout_cfg is not None:
                _stream_read_timeout = _provider_timeout_cfg
            else:
                _stream_read_timeout = float(os.getenv("GENGAR_STREAM_READ_TIMEOUT", 120.0))
                # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
                # prefill on large contexts before producing the first token.
                # Auto-increase the httpx read timeout unless the user explicitly
                # overrode GENGAR_STREAM_READ_TIMEOUT.
        ...
        def _call():
            ...
            _max_stream_retries = int(os.getenv("GENGAR_STREAM_RETRIES", 2))
        ...
        _stream_stale_timeout_base = float(os.getenv("GENGAR_STREAM_STALE_TIMEOUT", 180.0))
        # Local providers (Ollama, oMLX, llama-cpp) can take 300+ seconds
        # for prefill on large contexts.  Disable the stale detector unless
        # the user explicitly set GENGAR_STREAM_STALE_TIMEOUT.
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        if _is_qwen:
            _qwen_meta = {
                "sessionId": self.session_id or "gengar",
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        """
        if base_url_host_matches(self._base_url_lower, "jamesfincher.com"):
            return True
    ...
    def _compress_context(self, messages: list, system_message: str, *, approx_tokens: int = None, task_id: str = "default", focus_topic: str = None) -> tuple:
        ...
        # compression (not a fresh /new). Plugin engines (e.g. gengar-lcm) use
        # boundary_reason="compression" to preserve DAG lineage across the
        # rollover instead of re-initializing fresh per-session state.
        # See gengar-lcm#68. Built-in ContextCompressor ignores kwargs.
        try:
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        try:
            ...
            if _is_nous:
                summary_extra_body["tags"] = ["product=gengar"]
    ...
    def run_conversation(
        ...
        # ``gengar logs --session <id>`` can filter a single conversation.
        ...
        # are reused.  The system prompt is Gengar's territory; plugins
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            # cache prefix.  The system prompt is reserved for Gengar internals.
            if effective_system:
                api_messages = [{"role": "system", "content": effective_system}] + api_messages
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    if env_var_enabled("GENGAR_DUMP_REQUESTS"):
                    ...
                    if (
                        ...
                        print(f"{self.log_prefix}     • Re-authenticate: gengar login --provider nous")
                    ...
                    if (
                        ...
                        from hermes_constants import display_hermes_home as _dhh_fn
                        _dhh = _dhh_fn()
                        print(f"{self.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Gengar-managed OAuth/setup tokens")
                        print(f"{self.log_prefix}     • Check ANTHROPIC_API_KEY in {_dhh}/.env for API keys or legacy token values")
                        print(f"{self.log_prefix}     • For API keys: verify at https://platform.claude.com/settings/keys")
                        print(f"{self.log_prefix}     • For Claude Code: run 'claude /login' to refresh, then retry")
                        print(f"{self.log_prefix}     • Legacy cleanup: gengar config set ANTHROPIC_TOKEN \"\"")
                        print(f"{self.log_prefix}     • Clear stale keys: gengar config set ANTHROPIC_API_KEY \"\"")
                    ...
                    # providers (DeepSeek, Kimi, MiMo, Gengar).  A 429 can
                    ...
                    if is_client_error:
                        ...
                        if classified.is_auth or classified.reason == FailoverReason.billing:
                            if _provider == "openai-codex" and status_code == 401:
                                ...
                                self._vprint(f"{self.log_prefix}      2. Then run `gengar auth` to re-authenticate.", force=True)
                            else:
                                self._vprint(f"{self.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                                self._vprint(f"{self.log_prefix}      • Is the key valid? Run: gengar setup", force=True)
