import type { RunExternalProcess } from '@gengar/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`gengar ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching gengar: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`gengar ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
