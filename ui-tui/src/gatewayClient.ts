...
const STARTUP_TIMEOUT_MS = Math.max(5000, parseInt(process.env.GENGAR_TUI_STARTUP_TIMEOUT_MS ?? '15000', 10) || 15000)
const REQUEST_TIMEOUT_MS = Math.max(30000, parseInt(process.env.GENGAR_TUI_RPC_TIMEOUT_MS ?? '120000', 10) || 120000)
...
const resolvePython = (root: string) => {
  const configured = process.env.GENGAR_PYTHON?.trim() || process.env.PYTHON?.trim()
...
export class GatewayClient extends EventEmitter {
  ...
  start() {
    const root = process.env.GENGAR_PYTHON_SRC_ROOT ?? resolve(import.meta.dirname, '../../')
    const python = resolvePython(root)
    const cwd = process.env.GENGAR_CWD || root
