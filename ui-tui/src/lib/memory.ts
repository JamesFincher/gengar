...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.GENGAR_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.gengar', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `gengar-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
