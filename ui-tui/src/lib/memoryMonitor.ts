...
// Deferred @gengar/ink import: loading `@gengar/ink` at module top-level
...
async function _ensureEvictInkCaches(): Promise<(level: 'all' | 'half') => unknown> {
  ...
  _evictInkCachesPromise ??= import('@gengar/ink')
    .then(mod => {
...
export function startMemoryMonitor({
  ...
  const tick = async () => {
    ...
    // Deferred import keeps `@gengar/ink` off the cold-start critical path;
