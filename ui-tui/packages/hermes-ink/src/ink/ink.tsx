  ...
  async copySelectionNoClear(): Promise<string> {
    ...
    if (text) {
      try {
        ...
        if (process.env.GENGAR_TUI_DEBUG_CLIPBOARD) {
          console.error(
            '[clipboard] no path reached the clipboard (headless + no tmux?) — set GENGAR_TUI_FORCE_OSC52=1 to force the escape sequence'
          )
        }
      } catch (err) {
        if (process.env.GENGAR_TUI_DEBUG_CLIPBOARD) {
