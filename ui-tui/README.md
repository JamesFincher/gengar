...
React + Ink terminal UI for Gengar. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `GENGAR_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.gengar/.gengar_history` or under `GENGAR_HOME`.
...
```text
ui-tui/
  packages/gengar-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      gengar-ink.d.ts    type declarations for @gengar/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent GengarCLI subprocess for slash commands
```
