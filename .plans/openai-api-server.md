# OpenAI-Compatible API Server for Gengar
...
SSE streaming. By exposing this endpoint, gengar becomes instantly
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.gengar/.env`
2. Run `gengar gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with gengar through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns gengar as a model) |
...
Authorization: Bearer gengar-api-key-here
...
But gengar sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "gengar"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured gengar model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of gengar's core prompt
...
Once implemented, gengar works as a drop-in backend for:
