...
def _get_hermes_version() -> str:
    """Return the gengar package version, or 'dev' if unavailable."""
    try:
        from importlib.metadata import version
        return version("gengar")
    except Exception:
        return "dev"
...
def build_user_agent() -> str:
    ...
    Format::
        ...
        QQBotAdapter/<qqbot_version> (Python/<py_version>; <os>; Gengar/<hermes_version>)

    Example::

        QQBotAdapter/1.0.0 (Python/3.11.15; darwin; Gengar/0.9.0)
    """
    py_version = f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"
    os_name = platform.system().lower()
    hermes_version = _get_hermes_version()
    return f"QQBotAdapter/{QQBOT_VERSION} (Python/{py_version}; {os_name}; Gengar/{hermes_version})"
