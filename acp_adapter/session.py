"""ACP session manager — maps ACP sessions to Gengar AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.gengar/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Gengar itself is running in WSL.

    Windows ACP clients can launch ``gengar acp`` inside WSL while still sending
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Gengar from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["gengar-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Gengar agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Gengar AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Gengar runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.gengar/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``GENGAR_HOME`` dynamically rather than relying on
