"""ACP permission bridging — maps ACP approval requests to gengar approval callbacks."""
...
# Maps ACP PermissionOptionKind -> gengar approval result strings
_KIND_TO_GENGAR = {
    "allow_once": "once",
    "allow_always": "always",
    "reject_once": "deny",
...
def make_approval_callback(
    ...
    """
    Return a gengar-compatible ``approval_callback(command, description) -> str``
        ...
        if isinstance(outcome, AllowedOutcome):
            ...
            for opt in options:
                if opt.option_id == option_id:
                    return _KIND_TO_GENGAR.get(opt.kind, "deny")
