...
Thank you for contributing to Gengar! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Gengar to work everywhere.
...
Bundled skills (in `skills/`) ship with every Gengar install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `gengar skills browse` (labeled "official") and install it with `gengar skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [James Fincher Discord](https://github.com/jamesfincher/gengar). Users can install it with `gengar skills install`.
...
```bash
git clone --recurse-submodules https://github.com/jamesfincher/gengar.git
cd gengar
...
```bash
mkdir -p ~/.gengar/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.gengar/config.yaml
touch ~/.gengar/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.gengar/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/gengar" ~/.local/bin/gengar
...
├── cli.py                    # GengarCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (gengar-cli, gengar-telegram, etc.)
...
│
├── skills/                   # Bundled skills (copied to ~/.gengar/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (gengar.jamesfincher.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.gengar/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.gengar/`)

| Path | Purpose |
|------|---------|
| `~/.gengar/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.gengar/.env` | API keys and secrets |
| `~/.gengar/auth.json` | OAuth credentials (Nous Portal) |
| `~/.gengar/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.gengar/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.gengar/state.db` | SQLite session database |
| `~/.gengar/sessions/` | JSON session logs |
| `~/.gengar/cron/` | Scheduled job data |
| `~/.gengar/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`hermes_state.py`) with full-text search and unique session titles. JSON logs go to `~/.gengar/sessions/`.
...
Four fields are supported under `metadata.gengar`:
...
The user may skip setup and keep loading the skill. Gengar only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `gengar setup` or update `~/.gengar/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Gengar tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `gengar --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Gengar uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.gengar/skins/<name>.yaml`:
...
Gengar runs on Linux, macOS, and WSL2 on Windows. When writing code that touches the OS:
...
Gengar has terminal access. Security matters.
...
2. **Test manually**: Run `gengar` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/jamesfincher/gengar/issues)
- Include: OS, Python version, Gengar version (`gengar version`), full error traceback
...
- **Discord**: [discord.gg/jamesfincher](https://github.com/jamesfincher/gengar) — for questions, showcasing projects, and sharing skills
