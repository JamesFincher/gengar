#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `gengar kanban list` and
...
def hermes_available() -> bool:
    return shutil.which("gengar") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed gengar CLI."""
    try:
        out = subprocess.run(
            ["gengar", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `gengar kanban list`
    out = subprocess.run(
        ["gengar", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["gengar", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not hermes_available():
        print("ERROR: 'gengar' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
