...
the actual `setup.sh` that creates the project workspace, configures Gengar
...
> [jamesfincher/kanban-video-pipeline](https://github.com/jamesfincher/kanban-video-pipeline).
...
2. **Create profiles** — `gengar profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.gengar/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `gengar kanban create` assigned to the director
...
```bash
gengar profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.gengar/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.gengar/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Gengar
...
Each profile gets a `SOUL.md` at `~/.gengar/profiles/<name>/SOUL.md` that
...
```bash
gengar kanban create "Direct production of <video title>" \
    --assignee director \
...
`~/.gengar/.env` and macOS Keychain (if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    if grep -q "^${var}=" ~/.gengar/.env 2>/dev/null && \
       [ -n "$(grep "^${var}=" ~/.gengar/.env | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ~/.gengar/.env or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY gengar ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY gengar OPENROUTER_API_KEY || exit 1
# ...
```
