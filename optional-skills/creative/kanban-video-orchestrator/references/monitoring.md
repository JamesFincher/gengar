...
gengar kanban watch --tenant <project-slug>

# Snapshot of the board
gengar kanban list --tenant <project-slug>
gengar kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
gengar kanban stats --tenant <project-slug>

# Visual dashboard (browser)
gengar dashboard

# Inspect a specific task (includes comments + events)
gengar kanban show <task-id>

# Follow a single task's event stream
gengar kanban tail <task-id>
```

Verify available subcommands with `gengar kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `gengar kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `gengar kanban block <id>` then `gengar kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `gengar kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `gengar kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `gengar profile list` (profile exists?), `gengar status` (gateway/dispatcher up?), and verify tenant. |
...
gengar kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
gengar kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(gengar kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
gengar kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
gengar kanban block <task-id>
gengar kanban archive <task-id>

# Diagnose what happened
gengar kanban show <task-id>      # task body, comments, recent events
gengar kanban tail <task-id>      # follow the live event stream
gengar kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `gengar kanban create` for the director
...
while true; do
    clear
    gengar kanban list --tenant <slug>
    echo "---"
    gengar kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `gengar kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `gengar profile list` and start
