...
const SESSION_HEADER = "X-Gengar-Session-Token";
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__GENGAR_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Gengar dashboard server");
}
...
export const api = {
  ...
  updateGengar: () =>
    fetchJSON<ActionResponse>("/api/gengar/update", { method: "POST" }),
