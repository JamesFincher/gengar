...
2. **Gengar spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.gengar/cron/output/` and you can read them anytime.
:::
...
Gengar will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Gengar
```
...
Before creating cron jobs, ensure Gengar has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Gengar what you want:
...
Gengar will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Gengar to delegate each topic to a sub-agent:
...
Gengar will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
