...
Copy-paste recipes for common automation patterns. Each template uses Gengar's built-in [cron scheduler](/docs/user-guide/features/cron) for time-based triggers and [webhook platform](/docs/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`gengar webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `gengar webhook subscribe`) |
...
```bash
gengar cron create "0 2 * * *" \
  "You are a project manager triaging the jamesfincher/gengar GitHub repo.

1. Run: gh issue list --repo jamesfincher/gengar --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
gengar webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
gengar cron create "0 9 * * 1" \
  "Scan the jamesfincher/gengar repo for documentation drift.

1. Run: gh pr list --repo jamesfincher/gengar --state merged --json number,title,files,mergedAt --limit 30
...
```bash
gengar cron create "0 6 * * *" \
  "Run a dependency security audit on the gengar project.

1. cd ~/.gengar/gengar && source .venv/bin/activate
...
```bash
gengar webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
gengar webhook subscribe alert-triage \
...
```python title="~/.gengar/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Gengar-Monitor/1.0"})
...
gengar cron create "every 30m" \
  ...
  --script ~/.gengar/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
gengar cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Gengar development." \
...
```bash
gengar webhook subscribe github-issues \
  --events "issues" \
...
```bash
gengar webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
gengar webhook subscribe stripe-payments \
...
```bash
gengar cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the gengar codebase.
