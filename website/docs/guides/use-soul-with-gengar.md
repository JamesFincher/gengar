---
sidebar_position: 7
title: "Use SOUL.md with Gengar"
description: "How to use SOUL.md to shape Gengar's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Gengar

`SOUL.md` is the **primary identity** for your Gengar instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Gengar to feel like the same assistant every time you talk to it — or if you want to replace the Gengar persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Gengar should be
- what Gengar should avoid stylistically
- how Gengar should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Gengar is and how Gengar speaks
...
Gengar now uses only the global SOUL file for the current instance:

```text
~/.gengar/SOUL.md
```

If you run Gengar with a custom home directory, it becomes:
...
Gengar automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Gengar does not overwrite it
- if the file exists but is empty, Gengar adds nothing from it to the prompt

## How Gengar uses it

When Gengar starts a session, it reads `SOUL.md` from `GENGAR_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Gengar feels.
...
Gengar already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Gengar should sound.

# Avoid
What Gengar should not do.

# Defaults
How Gengar should behave when ambiguity appears.
```
...
Then restart Gengar or start a new session.
...
### I edited SOUL.md but Gengar still sounds the same

Check:
- you edited `~/.gengar/SOUL.md` or `$GENGAR_HOME/SOUL.md`
...
### Gengar is ignoring parts of my SOUL.md
