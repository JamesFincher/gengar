...
description: "Diagnose and fix common Gengar cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`gengar gateway` or `gengar serve`). For one-off debugging, you can manually trigger a tick with `gengar cron tick`.
...
```bash
date
gengar cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.gengar/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.gengar/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.gengar/.env` |
...
| `local` | Write access to `~/.gengar/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `gengar cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `gengar skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `gengar skills list`.
...
2. `~/.gengar/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `gengar cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Gengar config directory). Verify:
```bash
ls ~/.gengar/scripts/your-script.py   # Must exist
gengar cron edit <job_id> --script ~/.gengar/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `gengar skills install <skill-name>`.
...
Jobs are stored in `~/.gengar/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.gengar/cron/jobs.json
chmod 600 ~/.gengar/cron/jobs.json   # Your user should own it
```
...
```bash
gengar cron list                    # Show all jobs, states, next_run times
gengar cron run <job_id>            # Schedule for next tick (for testing)
gengar cron edit <job_id>           # Fix configuration issues
gengar logs                         # View recent Gengar logs
gengar skills list                  # Verify installed skills
```
...
1. Run the job with `gengar cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.gengar/logs/agent.log` for scheduler messages and `~/.gengar/logs/errors.log` for warnings
3. Open an issue at [github.com/jamesfincher/gengar](https://github.com/jamesfincher/gengar) with:
