...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Gengar  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Gengar instantly when PRs are opened or updated.
:::
...
- **Gengar installed** — see the [Installation guide](/docs/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  gengar gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.gengar/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Gengar can access GitHub. Start a chat:
...
```
Run: gh pr list --repo jamesfincher/gengar --state open --limit 3
```
...
Still in the chat, ask Gengar to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo jamesfincher/gengar
```
...
A skill gives Gengar consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.gengar/skills/code-review
```

Create `~/.gengar/skills/code-review/SKILL.md`:
...
Verify it loaded — start `gengar` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Gengar your team's standards:
...
2. Teach Gengar your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
gengar gateway status    # Is the gateway running?
gengar cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/docs/guides/build-a-gengar-plugin)** — wrap the review logic into a shareable plugin
