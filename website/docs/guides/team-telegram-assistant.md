...
This tutorial walks you through setting up a Telegram bot powered by Gengar that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Gengar installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.gengar/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Gengar itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Gengar Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_gengar_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Gengar. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Gengar Gateway...
...
# View live logs
journalctl --user -u gengar-gateway -f
...
gengar gateway start
gengar gateway stop
tail -f ~/.gengar/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `gengar gateway install` to update the plist.
:::
...
```bash
gengar gateway stop && gengar gateway start
```
...
gengar pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.gengar/.env`:
...
Control how much detail the bot shows when using tools. In `~/.gengar/config.yaml`:
...
Customize how the bot communicates by editing `~/.gengar/SOUL.md`:

For a full guide, see [Use SOUL.md with Gengar](/docs/guides/use-soul-with-gengar).
...
```bash
# From the CLI
gengar cron list          # View all scheduled jobs
gengar cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u gengar-gateway` (Linux) or `~/.gengar/logs/gateway.log` (macOS) |
| Cron job output | `~/.gengar/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.gengar/cron/jobs.json` |
| Pairing data | `~/.gengar/pairing/` |
| Session history | `~/.gengar/sessions/` |
