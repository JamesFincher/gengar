---
sidebar_position: 6
title: "Use MCP with Gengar"
description: "A practical guide to connecting MCP servers to Gengar, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Gengar

This guide shows how to actually use MCP with Gengar in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Gengar tool
- you want Gengar to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Gengar to internal APIs, databases, or company systems without modifying Gengar core

Do not use MCP when:
- a built-in Gengar tool already solves the job well
...
- Gengar remains the agent
- MCP servers contribute tools
- Gengar discovers those tools at startup or reload time
...
If you installed Gengar with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Gengar banner/status should show MCP integration when configured
- ask Gengar what tools it has available
...
There are two categories of MCP-exposed functionality in Gengar:
...
2. Gengar-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Gengar will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Gengar to reason over a bounded workspace.
...
Now Gengar can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Gengar core.
...
Because Gengar now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
