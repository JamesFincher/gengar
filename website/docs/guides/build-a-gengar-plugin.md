...
title: "Build a Gengar Plugin"
description: "Step-by-step guide to building a complete Gengar plugin with tools, hooks, data files, and skills"
---

# Build a Gengar Plugin

This guide walks through building a complete Gengar plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.
...
```bash
mkdir -p ~/.gengar/plugins/calculator
cd ~/.gengar/plugins/calculator
```
...
This tells Gengar: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** Gengar may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `gengar my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Gengar continues fine
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.gengar/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.gengar/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `gengar plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), Gengar injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.
...
- **The system prompt is Gengar's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `gengar <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for gengar my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: gengar my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for gengar my-plugin."""
...
After registration, users can run `gengar my-plugin status`, `gengar my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `gengar name` in a terminal |
...
```bash
pip install gengar-plugin-calculator
# Plugin auto-discovered on next gengar startup
```
...
```python
# Wrong — will break if Gengar passes extra context
def handler(args):
    ...
