...
description: "Run Gengar directly on an Android phone with Termux"
---

# Gengar on Android with Termux

This is the tested path for running Gengar directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Gengar from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Gengar now ships a Termux-aware installer path:
...
- links `gengar` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `gengar` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/gengar" "$PREFIX/bin/gengar"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `gengar` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.gengar/.env`.
...
### `gengar doctor` says ripgrep or Node is missing
