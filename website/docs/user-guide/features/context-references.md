...
Type `@` followed by a reference to inject content directly into your message. Gengar expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Gengar env: `$GENGAR_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$GENGAR_HOME/skills/.hub/`
