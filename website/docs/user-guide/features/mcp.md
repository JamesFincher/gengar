...
description: "Connect Gengar to external tool servers via MCP — and control exactly which MCP tools Gengar loads"
---

# MCP (Model Context Protocol)

MCP lets Gengar connect to external tool servers so the agent can use tools that live outside Gengar itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Gengar to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native Gengar tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Gengar to see
...
2. Add an MCP server to `~/.gengar/config.yaml`:
...
4. Ask Gengar to use the MCP-backed capability.
...
Gengar will discover the MCP server's tools and use them like any other tool.
...
HTTP MCP servers are remote endpoints Gengar connects to directly.
...
- you do not want Gengar spawning a local subprocess for that integration

## Basic configuration reference

Gengar reads MCP config from `~/.gengar/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Gengar skips the server entirely |
...
## How Gengar registers MCP tools

Gengar prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Gengar sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Gengar also registers utility tools around MCP resources and prompts:
...
- Gengar only registers resource utilities if the MCP session actually supports resource operations
- Gengar only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Gengar, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Gengar skips the server completely and does not even attempt a connection.
...
You can also separately disable Gengar-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Gengar does not create an empty runtime MCP toolset for that server.
...
Gengar discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Gengar when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Gengar receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Gengar does not blindly pass your full shell environment.
...
Then verify your config and restart Gengar.
...
Because Gengar now only registers those wrappers when both are true:
...
MCP servers can request LLM inference from Gengar via the `sampling/createMessage` protocol. This allows an MCP server to ask Gengar to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Gengar as an MCP server

In addition to connecting **to** MCP servers, Gengar can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Gengar's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Gengar
- You want a single MCP server that bridges to all of Gengar's connected messaging platforms at once
- You already have a running Gengar gateway with connected platforms
...
Add Gengar to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Gengar in a specific location:

```json
{
  "mcpServers": {
    "gengar": {
      "command": "/home/user/.gengar/gengar/venv/bin/gengar",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a Gengar-specific channel browser:
...
The MCP server includes a live event bridge that polls Gengar's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
gengar mcp serve              # Normal mode
gengar mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from Gengar's session store (`~/.gengar/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the Gengar agent itself.
