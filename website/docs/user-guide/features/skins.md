...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the Gengar CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the Gengar CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
/skin mytheme        # switch to a custom skin from ~/.gengar/skins/mytheme.yaml
```

Or set the default skin in `~/.gengar/config.yaml`:
...
|------|-------------|----------------|------------------|
| `default` | Classic Gengar — gold and kawaii | `Gengar` | Warm gold borders, cornsilk text, kawaii faces in spinners. The familiar caduceus banner. Clean and inviting. |
...
| `mono` | Monochrome — clean grayscale | `Gengar` | All grays — no color. Borders are `#555555`, text is `#c9d1d9`. Ideal for minimal terminal setups or screen recordings. |
| `slate` | Cool blue — developer-focused | `Gengar` | Royal blue borders (`#4169e1`), soft blue text. Calm and professional. No custom spinner — uses default faces. |
| `daylight` | Light theme for bright terminals with dark text and cool blue accents | `Gengar` | Designed for white or bright terminals. Dark slate text with blue borders, pale status surfaces, and a light completion menu that stays readable in light terminal profiles. |
| `warm-lightmode` | Warm brown/gold text for light terminal backgrounds | `Gengar` | Warm parchment tones for light terminals. Dark brown text with saddle-brown accents, cream-colored status surfaces. An earthy alternative to the cooler daylight theme. |
...
|-----|-------------|---------|
| `agent_name` | Name shown in banner title and status display | `Gengar` |
| `welcome` | Welcome message shown at CLI startup | `Welcome to Gengar! Type your message or /help for commands.` |
| `goodbye` | Message shown on exit | `Goodbye! ⚕` |
| `response_label` | Label on the response box header | ` ⚕ Gengar ` |
...
| `banner_logo` | string | Rich-markup ASCII art logo (replaces the default GENGAR_AGENT banner) | `""` |
| `banner_hero` | string | Rich-markup hero art (replaces the default caduceus art) | `""` |

## Custom skins

Create YAML files under `~/.gengar/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.

### Full custom skin YAML template

```yaml
# ~/.gengar/skins/mytheme.yaml
...
## Gengar Mod — Visual Skin Editor

[Gengar Mod](https://github.com/cocktailpeanut/gengar-mod) is a community-built web UI for creating and managing skins visually. Instead of writing YAML by hand, you get a point-and-click editor with live preview.

![Gengar Mod skin editor](https://raw.githubusercontent.com/cocktailpeanut/gengar-mod/master/nous.png)

**What it does:**

- Lists all built-in and custom skins
- Opens any skin into a visual editor with all Gengar skin fields (colors, spinner, branding, tool prefix, tool emojis)
...
- Saves directly to `~/.gengar/skins/`
- Activates a skin by updating `~/.gengar/config.yaml`
...
```bash
git clone https://github.com/cocktailpeanut/gengar-mod.git
cd gengar-mod/app
npm install
npm start
```
...
6. Click **Save** to write the skin YAML to `~/.gengar/skins/`.
7. Click **Activate** to set it as the current skin (updates `display.skin` in `config.yaml`).

Gengar Mod respects the `GENGAR_HOME` environment variable, so it works with [profiles](/docs/user-guide/profiles) too.
...
- User skins in `~/.gengar/skins/` take precedence over built-in skins with the same name.
