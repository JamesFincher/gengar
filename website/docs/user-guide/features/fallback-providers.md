...
Gengar has three layers of resilience that keep your sessions running when providers hit issues:
...
When your main LLM provider encounters errors — rate limits, server overload, auth failures, connection drops — Gengar can automatically switch to a backup provider:model pair mid-session without losing your conversation.
...
`gengar fallback` reuses the provider picker from `gengar model` — same provider list, same credential prompts, same validation. Press `a` to add a fallback, `↑`/`↓` to reorder, `d` to remove, `q` to save and exit. Changes persist under `model.fallback_providers` in `config.yaml`.

If you'd rather edit the YAML directly, add a `fallback_model` section to `~/.gengar/config.yaml`:
...
`fallback_model` (singular) is the legacy single-fallback key — Gengar still honors it for back-compat. `fallback_providers` (plural, list) supports multiple fallbacks tried in order; `gengar fallback` writes to this key. When both are set, Gengar merges them with `fallback_providers` taking priority.
:::
...
| Nous Portal | `nous` | `gengar auth` (OAuth) |
| OpenAI Codex | `openai-codex` | `gengar model` (ChatGPT OAuth) |
...
| Google Gemini (OAuth) | `google-gemini-cli` | `gengar model` (Google OAuth; optional: `GENGAR_GEMINI_PROJECT_ID`) |
...
| Qwen Portal (OAuth) | `qwen-oauth` | `gengar model` (Qwen Portal OAuth; optional: `GENGAR_QWEN_BASE_URL`) |
| MiniMax (OAuth) | `minimax-oauth` | `gengar model` (MiniMax portal OAuth) |
...
Gengar uses separate lightweight models for side tasks. Each task has its own provider resolution chain that acts as a built-in fallback system.
...
When a task's provider is set to `"auto"` (the default), Gengar tries providers in order until one works:
...
If the resolved provider fails at call time, Gengar also has an internal retry: if the provider is not OpenRouter and no explicit `base_url` is set, it tries OpenRouter as a last-resort fallback.
...
For `auxiliary.session_search`, Gengar also supports:
...
| `"codex"` | Force Codex OAuth | `gengar model` → Codex |
...
`base_url` takes precedence over `provider`. Gengar uses the configured `api_key` for authentication, falling back to `OPENAI_API_KEY` if not set. It does **not** reuse `OPENROUTER_API_KEY` for custom endpoints.
...
If no provider is available for compression, Gengar drops middle conversation turns without generating a summary rather than failing the session.
