...
description: "Customize Gengar's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Gengar's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `GENGAR_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Gengar is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Gengar now seeds a default `SOUL.md` automatically in:

```text
~/.gengar/SOUL.md
```

More precisely, it uses the current instance's `GENGAR_HOME`, so if you run Gengar with a custom home directory, it will use:
...
- Gengar creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Gengar loads `SOUL.md` only from `GENGAR_HOME`
- Gengar does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Gengar falls back to a built-in default identity
...
If Gengar loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `GENGAR_HOME`, the personality belongs to the Gengar instance itself.

That also makes it easier to teach users:
- "Edit `~/.gengar/SOUL.md` to change Gengar' default personality."
...
- how Gengar should handle uncertainty, disagreement, or ambiguity
...
## What Gengar injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Gengar falls back to a built-in default identity ("You are Gengar, an intelligent AI assistant created by James Fincher..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Gengar ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Gengar, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Gengar its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.gengar/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.gengar/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-gengar)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Gengar speaks
- `display.skin` and `/skin` affect how Gengar looks in the terminal
