...
title: "Plugins"
description: "Extend Gengar with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Gengar has a plugin system for adding custom tools, hooks, and integrations without modifying core code.

**→ [Build a Gengar Plugin](/docs/guides/build-a-gengar-plugin)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.gengar/plugins/` with a `plugin.yaml` and Python code:
...
Start Gengar — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.gengar/plugins/hello-world/plugin.yaml`**
...
**`~/.gengar/plugins/hello-world/__init__.py`**

```python
"""Minimal Gengar plugin — registers a tool and a hook."""
...
Drop both files into `~/.gengar/plugins/hello-world/`, restart Gengar, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.gengar/plugins/` are disabled by default. Enable them only for trusted repositories by setting `GENGAR_ENABLE_PROJECT_PLUGINS=true` before starting Gengar.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `gengar <plugin> <subcommand>` |
...
**Every plugin — user-installed, bundled, or pip — is disabled by default.** Discovery finds them (so they show up in `gengar plugins` and `/plugins`), but nothing loads until you add the plugin's name to `plugins.enabled` in `~/.gengar/config.yaml`. This stops anything with hooks or tools from running without your explicit consent.
...
```bash
gengar plugins                    # interactive toggle (space to check/uncheck)
gengar plugins enable <name>      # add to allow-list
gengar plugins disable <name>     # remove from allow-list + add to disabled
```

After `gengar plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

### Migration for existing users

When you upgrade to a version of Gengar that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.gengar/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled plugins are NOT grandfathered — even existing users have to opt in explicitly.
...
Gengar has three kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.gengar/plugins/` |
...
On NixOS, plugins can be installed declaratively via the module options — no `gengar plugins install` needed. See the **[Nix Setup guide](/docs/getting-started/nix-setup#plugins)** for full details.
...
```bash
gengar plugins                               # unified interactive UI
gengar plugins list                          # table: enabled / disabled / not enabled
gengar plugins install user/repo             # install from Git, then prompt Enable? [y/N]
gengar plugins install user/repo --enable    # install AND enable (no prompt)
gengar plugins install user/repo --no-enable # install but leave disabled (no prompt)
gengar plugins update my-plugin              # pull latest
gengar plugins remove my-plugin              # uninstall
gengar plugins enable my-plugin              # add to allow-list
gengar plugins disable my-plugin             # remove from allow-list + add to disabled
```

### Interactive UI

Running `gengar plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `gengar plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
See the **[full guide](/docs/guides/build-a-gengar-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
