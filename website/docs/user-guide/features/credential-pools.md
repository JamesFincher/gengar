...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Gengar automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, Gengar auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
gengar auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
gengar auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
gengar auth add anthropic --type oauth
...
anthropic (3 credentials):
  #1  gengar_pkce          oauth   gengar_pkce ←
...
Run `gengar auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `gengar auth` | Interactive pool management wizard |
| `gengar auth list` | Show all pools and credentials |
| `gengar auth list <provider>` | Show a specific provider's pool |
| `gengar auth add <provider>` | Add a credential (prompts for type and key) |
| `gengar auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `gengar auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `gengar auth remove <provider> <index>` | Remove credential by 1-based index |
| `gengar auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `gengar auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `gengar model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via gengar model:
gengar auth list
# Shows:
...
gengar auth add Together.ai --api-key sk-together-second-key
```
...
Gengar automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| Gengar PKCE OAuth | `~/.gengar/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `gengar auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `gengar auth add`) are never auto-pruned.
...
Pool state is stored in `~/.gengar/auth.json` under the `credential_pool` key:
