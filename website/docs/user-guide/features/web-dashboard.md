...
The web dashboard is a browser-based UI for managing your Gengar installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.
...
| `--tui` | off | Expose the in-browser Chat tab (embedded `gengar --tui` via PTY/WebSocket). Alternatively set `GENGAR_DASHBOARD_TUI=1`. |

```bash
# Custom port
gengar dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
gengar dashboard --host 0.0.0.0

# Start without opening browser
gengar dashboard --no-open
```

## Prerequisites

The default `gengar` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
pip install 'gengar[web,pty]'
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `pip install gengar[all]` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `gengar dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.
...
- Node.js (same requirement as `gengar --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`pip install 'gengar[web,pty]'`, or `[all]` covers both)
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `gengar config set` and the gateway read from.
:::
...
Browse, search, and toggle skills and toolsets. Skills are loaded from `~/.gengar/skills/` and grouped by category.
...
This re-reads `~/.gengar/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
When you run `gengar update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `gengar dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Gengar Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
