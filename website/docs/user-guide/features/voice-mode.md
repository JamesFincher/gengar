...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Gengar — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Gengar supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Gengar](/docs/guides/use-voice-mode-with-gengar).

## Prerequisites

Before using voice features, make sure you have:

1. **Gengar installed** — `pip install gengar` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `gengar model` or set your preferred provider credentials in `~/.gengar/.env`
3. **A working base setup** — run `gengar` to verify the agent responds to text before enabling voice

:::tip
The `~/.gengar/` directory and default `config.yaml` are created automatically the first time you run `gengar`. You only need to create `~/.gengar/.env` manually for API keys.
:::
...
pip install "gengar[messaging]"

# Premium TTS (ElevenLabs)
pip install "gengar[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "gengar[all]"
```
...
Voice mode is available in both the **classic CLI** (`gengar chat`) and the **TUI** (`gengar --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.gengar/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
gengar                # Start the interactive CLI
```
...
1. Start the CLI with `gengar` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.gengar/config.yaml` (default: `ctrl+b`).
:::
...
```bash
gengar gateway        # Start the gateway (connects to configured platforms)
gengar gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@gengarbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.gengar/.env`:
```bash
...
```bash
gengar gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.gengar/.env`
...
- Review gateway logs: `tail -f ~/.gengar/logs/gateway.log`
