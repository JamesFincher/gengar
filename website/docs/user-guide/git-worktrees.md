...
title: "Git Worktrees"
description: "Run multiple Gengar agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Gengar is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Gengar so each session has a clean, isolated working directory.

## Why Use Worktrees with Gengar?

Gengar treats the **current working directory** as the project root:

- CLI: the directory where you run `gengar` or `gengar chat`
...
git worktree add ../repo-feature feature/gengar-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/gengar-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Gengar there:

```bash
cd ../repo-feature

# Start Gengar in the worktree
gengar
```
...
git worktree add ../repo-experiment-a feature/gengar-a
git worktree add ../repo-experiment-b feature/gengar-b
```
...
- Works on its own branch (`feature/gengar-a` vs `feature/gengar-b`).
...
- Gengar checkpoint data under `~/.gengar/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Gengar experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/gengar-checkpoints-docs`, `feature/gengar-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Gengar from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `gengar -w` (Automatic Worktree Mode)

Gengar has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `gengar/gengar-<hash>`).
...
```bash
gengar -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `gengar -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Gengar session its own clean checkout.
