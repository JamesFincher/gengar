...
| | |
|---|---|
| Source | Optional — install with `gengar skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Gengar |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Gengar terminal calls (desktop app flow only)
...
### Service Account (recommended for Gengar)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.gengar/.env` (the skill will prompt for this on first load).
...
## Gengar Execution Pattern (desktop app flow)

Gengar terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/gengar-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/gengar-op.sock"
