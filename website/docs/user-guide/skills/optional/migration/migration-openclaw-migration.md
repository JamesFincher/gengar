---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Gengar"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Gengar"
---
...
Migrate a user's OpenClaw customization footprint into Gengar. Imports Gengar-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `gengar skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Gengar (James Fincher) |
| License | MIT |
| Tags | `Migration`, `OpenClaw`, `Gengar`, `Memory`, `Persona`, `Import` |
| Related skills | [`gengar`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-gengar) |
...
# OpenClaw -> Gengar Migration

Use this skill when a user wants to move their OpenClaw setup into Gengar with minimal manual cleanup.
...
```bash
gengar claw migrate              # Full interactive migration
...
gengar claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `gengar setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_gengar.py` to:

- import `SOUL.md` into the Gengar home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Gengar memory entries
- merge OpenClaw command approval patterns into Gengar `command_allowlist`
- migrate Gengar-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.gengar/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Gengar workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.gengar/tts/`
- archive non-secret docs that do not have a direct Gengar destination
...
- `scripts/openclaw_to_gengar.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.gengar/skills/migration/openclaw-migration/scripts/openclaw_to_gengar.py`

Do not guess a shorter path like `~/.gengar/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.gengar/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Gengar-compatible secrets, currently:
...
Gengar CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Gengar. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Gengar workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Gengar skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.gengar/skills/migration/openclaw-migration/scripts/openclaw_to_gengar.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.gengar/skills/migration/openclaw-migration/scripts/openclaw_to_gengar.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.gengar/skills/migration/openclaw-migration/scripts/openclaw_to_gengar.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.gengar/skills/migration/openclaw-migration/scripts/openclaw_to_gengar.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.gengar/skills/migration/openclaw-migration/scripts/openclaw_to_gengar.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.gengar/skills/migration/openclaw-migration/scripts/openclaw_to_gengar.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Gengar unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Gengar targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Gengar destination. Unsupported auth blobs must still be reported as skipped.
...
- Gengar persona state imported
- Gengar memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.gengar/skills/openclaw-imports/`
