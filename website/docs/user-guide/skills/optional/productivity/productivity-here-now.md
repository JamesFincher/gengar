...
| | |
|---|---|
| Source | Optional — install with `gengar skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${GENGAR_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${GENGAR_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${GENGAR_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client gengar
```
...
```bash
PUBLISH="${GENGAR_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client gengar
```
...
```bash
DRIVE="${GENGAR_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `gengar`)    |
