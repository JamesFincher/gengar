...
| | |
|---|---|
| Source | Optional — install with `gengar skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Gengar"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.gengar/.env`:
...
```bash
CANVAS="python $GENGAR_HOME/skills/productivity/canvas/scripts/canvas_api.py"
