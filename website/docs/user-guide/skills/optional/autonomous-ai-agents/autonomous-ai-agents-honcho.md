...
description: "Configure and use Honcho memory with Gengar -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Gengar -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `gengar skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Gengar

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Gengar profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/gengar#running-honcho-locally-with-gengar

### Verify

```bash
gengar honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Gengar profile's AI peer
...
Honcho models conversations as interactions between **peers**. Gengar creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Gengar instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Gengar run |
| `global` | Single session across all directories |

Manual override: `gengar honcho map my-project-name`
...
Each Gengar profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block gengar.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `gengar.coder` host block in `honcho.json`
...
```bash
gengar honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Gengar when Honcho memory is active.
...
Config file: `$GENGAR_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `gengar honcho setup`. Ensure `memory.provider: honcho` is in `~/.gengar/config.yaml`.

### Memory not persisting across sessions
Check `gengar honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `gengar profile create <name> --clone`. For existing profiles: `gengar honcho sync`.
...
| Command | Description |
|---------|-------------|
| `gengar honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `gengar honcho status` | Show resolved config, connection test, peer info for active profile |
| `gengar honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `gengar honcho disable` | Disable Honcho for the active profile |
| `gengar honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `gengar honcho peers` | Show peer identities across all profiles |
| `gengar honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `gengar honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `gengar honcho sessions` | List known directory-to-session-name mappings |
| `gengar honcho map <name>` | Map current working directory to a Honcho session name |
| `gengar honcho identity` | Seed AI peer identity or show both peer representations |
| `gengar honcho sync` | Create host blocks for all Gengar profiles that don't have one yet |
| `gengar honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Gengar + Honcho |
| `gengar memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `gengar memory status` | Show active memory provider and config |
| `gengar memory off` | Disable external memory provider |
