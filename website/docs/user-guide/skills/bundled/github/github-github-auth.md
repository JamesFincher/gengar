...
- Give it a title like "gengar-&lt;machine-name>"
...
elif [ -f ~/.gengar/.env ] && grep -q "^GITHUB_TOKEN=" ~/.gengar/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.gengar/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
