...
Connect Gengar to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Gengar supports two WeCom integration modes:
...
3. Gengar decrypts the message, queues it for the agent
...
(Use `gengar gateway start` only after `gengar gateway install` has registered the systemd/launchd service.)
