...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Gengar to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Gengar to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.gengar/.env`:
...
All standard Gengar commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.gengar/logs/gateway.log`
...
4. Consider enabling verbose logging: `GENGAR_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Gengar automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
GENGAR_LOG_LEVEL=debug gengar gateway
```
...
```bash
gengar chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
