...
sidebar_position: 4
title: "Slack"
description: "Set up Gengar as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Gengar to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Gengar instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Gengar uses the modern
...
The fastest path is to paste a manifest Gengar generates for you. It
...
### Option A: From a Gengar-generated manifest (recommended)

1. Generate the manifest:
   ```bash
   gengar slack manifest --write
   ```
   This writes `~/.gengar/slack-manifest.json` and prints paste-in
   instructions.
...
4. Enter an app name (e.g., "Gengar") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Gengar can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `gengar-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Gengar configuration issue.
:::
...
Gengar uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
gengar gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
gengar gateway              # Foreground
gengar gateway install      # Install as a user service
sudo gengar gateway install --system   # Linux only: boot-time system service
```
...
Every Gengar command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Gengar command with its description.

Under the hood: Gengar ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/jamesfincher/gengar/blob/main/hermes_cli/commands.py)
...
When Gengar adds new commands (e.g. after `gengar update`), regenerate
...
3. Paste the new contents of `~/.gengar/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/gengar <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/gengar btw run the tests` — Gengar routes it the same way as `/btw
run the tests`. Free-form questions also work: `/gengar what's the
...
```bash
gengar slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how Gengar behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Gengar what time is it?`). In channels, Gengar replies in a thread attached to that message. |
| **Threads** | If you @mention Gengar inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.gengar/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Gengar in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Gengar will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Gengar will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Gengar`).

---

## Multi-Workspace Support

Gengar can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Gengar also loads tokens from an **OAuth token file** at:

```
~/.gengar/slack_tokens.json
```
...
- When a message arrives, Gengar uses the correct workspace-specific client to respond.
...
Gengar supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Gengar` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Gengar` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Gengar now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Gengar`)
...
- Tokens should be stored in `~/.gengar/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Gengar config directory
