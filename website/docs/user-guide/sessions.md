...
Gengar automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.gengar/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.gengar/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`gengar` or `gengar chat`) |
...
gengar -c "my project"   # → resumes "my project #3"
```
...
gengar --resume 20250305_091523_a1b2c3d4
gengar -r 20250305_091523_a1b2c3d4

# Resume by title
gengar --resume "refactoring auth"

# Or with the chat subcommand
gengar chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `gengar sessions list`.

### Conversation Recap on Resume

When you resume a session, Gengar displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Gengar session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.gengar/config.yaml`:
...
Gengar automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `gengar sessions list` or `gengar sessions browse`.
...
```bash
gengar sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Gengar creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`gengar -c "my project"`), it automatically picks the most recent session in the lineage.
...
Gengar provides a full set of session management commands via `gengar sessions`:
...
# Filter by platform
gengar sessions list --source telegram

# Show more sessions
gengar sessions list --limit 50
```
...
gengar sessions export backup.jsonl

# Export sessions from a specific platform
gengar sessions export telegram-history.jsonl --source telegram

# Export a single session
gengar sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
gengar sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
gengar sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
gengar sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
gengar sessions prune

# Custom age threshold
gengar sessions prune --older-than 30

# Only prune sessions from a specific platform
gengar sessions prune --source telegram --older-than 60

# Skip confirmation
gengar sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`gengar insights`](/docs/reference/cli-commands#gengar-insights).
...
When Gengar cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Gengar uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Gengar in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.gengar/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.gengar/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.gengar/sessions/sessions.json` | Maps session keys to active session IDs |
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Gengar process in the same `GENGAR_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.gengar/config.yaml`:
...
gengar sessions delete <session_id>
