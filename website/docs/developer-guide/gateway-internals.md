...
The messaging gateway is the long-running process that connects Gengar to 14+ external messaging platforms through a unified architecture.
...
|--------|-----------------|
| `~/.gengar/.env` | API keys, bot tokens, platform credentials |
| `~/.gengar/config.yaml` | Model settings, tool configuration, display options |
...
Hooks are discovered from `gateway/builtin_hooks/` (always active) and `~/.gengar/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- `gengar gateway start` / `gengar gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.gengar/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `gengar gateway stop` stops only the current profile's gateway. `gengar gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
