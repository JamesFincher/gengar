...
description: "Build wrapper CLIs that extend the Gengar TUI with custom widgets, keybindings, and layout changes"
---

# Extending the CLI

Gengar exposes protected extension hooks on `GengarCLI` so wrapper CLIs can add widgets, keybindings, and layout customizations without overriding the 1000+ line `run()` method. This keeps your extension decoupled from internal changes.
...
```python
#!/usr/bin/env python3
"""my_cli.py — Example wrapper CLI that extends Gengar."""

from cli import GengarCLI
from prompt_toolkit.layout import FormattedTextControl, Window
from prompt_toolkit.filters import Condition


class MyCLI(GengarCLI):
...
Called after Gengar registers its own keybindings and before the layout is built. Add your keybindings to `kb`.
