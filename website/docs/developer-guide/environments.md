...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the Gengar Atropos integration"
---

# Environments, Benchmarks & Data Generation

Gengar includes a full environment framework that connects its tool-calling capabilities to the [Atropos](an external Atropos checkout) RL training framework. This enables three workflows:
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for Gengar/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
classDiagram
    ...
    BaseEnv <|-- GengarBaseEnv
    GengarBaseEnv <|-- TerminalTestEnv
    GengarBaseEnv <|-- GengarSweEnv
    GengarBaseEnv <|-- TerminalBench2EvalEnv
...
The gengar layer (`environments/hermes_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls gengar's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `GengarLoop` and scores the result
...
Your environment inherits from `GengarBaseEnv` and implements five methods:
...
`GengarLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as gengar's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any gengar tool |
...
parser = get_parser("gengar")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `gengar`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
Dataset: [jamesfincher/terminal-bench-2](https://huggingface.co/datasets/jamesfincher/terminal-bench-2) on HuggingFace.
...
TBLite is a thin subclass of TerminalBench2 — only the dataset and timeouts differ. Created by the OpenThoughts Agent team (Snorkel AI + Bespoke Labs). Dataset: [jamesfincher/openthoughts-tblite](https://huggingface.co/datasets/jamesfincher/openthoughts-tblite).
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"gengar"`, `"qwen"`, `"mistral"`)
...
class MyEnvConfig(GengarEnvConfig):
...
### GengarEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which gengar toolsets to enable |
...
| `tool_call_parser` | `str` | `"gengar"` | Parser for Phase 2 raw output |
...
env:
  ...
  dataset_name: "jamesfincher/terminal-bench-2"
  tokenizer_name: "jamesfincher/Gengar-3-Llama-3.1-8B"
  use_wandb: true
...
- Python >= 3.11
- `atroposlib`: `pip install git+an external Atropos checkout.git`
- An LLM API key (OpenRouter, OpenAI, or self-hosted VLLM/SGLang)

### For Modal-sandboxed benchmarks (TB2, TBLite)

- [Modal](https://modal.com) account and CLI: `pip install "gengar[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "gengar[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
├── hermes_base_env.py          # Abstract base class (GengarBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (GengarLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── hermes_parser.py        # Gengar/ChatML <tool_call> format
