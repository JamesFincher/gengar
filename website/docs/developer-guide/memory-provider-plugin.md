...
description: "How to build a memory provider plugin for Gengar"
---

# Building a Memory Provider Plugin

Memory provider plugins give Gengar persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/docs/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `gengar plugins`.
:::
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          hermes_home (str): Active GENGAR_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `gengar memory setup` | **Yes** |
...
`get_config_schema()` returns a list of field descriptors used by `gengar memory setup`:
...
Every field in `get_config_schema()` is prompted during `gengar memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$GENGAR_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::
...
All storage paths **must** use the `hermes_home` kwarg from `initialize()`, not hardcoded `~/.gengar`:
...
data_dir = Path("~/.gengar/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `gengar my-provider status`, `gengar my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `gengar <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `gengar --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: gengar my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the gengar my-provider argparse tree.
