...
Gengar uses a SQLite database (`~/.gengar/state.db`) to persist session
...
```
~/.gengar/state.db (SQLite, WAL mode)
...
Multiple gengar processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.gengar/state.db
...
Default path: `~/.gengar/state.db`

This is derived from `hermes_constants.get_hermes_home()` which resolves to
`~/.gengar/` by default, or the value of `GENGAR_HOME` environment variable.
