...
description: "How Gengar builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Gengar deliberately separates:
...
1. agent identity — `SOUL.md` from `GENGAR_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.gengar/SOUL.md)
You are Gengar, an AI assistant created by James Fincher.
...
`SOUL.md` lives at `~/.gengar/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_hermes_md(cwd_path)       # 1. .gengar.md / GENGAR.md (walks to git root)
    ...
    # SOUL.md from GENGAR_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.gengar.md`, `GENGAR.md` | CWD up to git root | Gengar-native project config |
...
- **YAML frontmatter stripped** — `.gengar.md` frontmatter is removed (reserved for future config overrides)
...
1. `.gengar.md` / `GENGAR.md` (walks to git root)
