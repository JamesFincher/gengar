...
description: "How Gengar resolves providers, credentials, API modes, and auxiliary models at runtime"
---

# Provider Runtime Resolution

Gengar has a shared provider runtime resolver used across:
...
That ordering matters because Gengar treats the saved model/provider choice as the source of truth for normal runs. This prevents a stale shell export from silently overriding the endpoint a user last selected in `gengar model`.
...
This resolver is the main reason Gengar can share auth/runtime logic between:
...
Set `AI_GATEWAY_API_KEY` in `~/.gengar/.env` and run with `--provider ai-gateway`. Gengar fetches available models from the gateway's `/models` endpoint, filtering to language models with tool-use support.

## OpenRouter, AI Gateway, and custom OpenAI-compatible base URLs

Gengar contains logic to avoid leaking the wrong API key to a custom endpoint when multiple provider keys exist (e.g. `OPENROUTER_API_KEY`, `AI_GATEWAY_API_KEY`, and `OPENAI_API_KEY`).
...
Gengar also distinguishes between:
...
When provider resolution selects `anthropic`, Gengar uses:
...
- Gengar preflights Anthropic credential refresh before native Messages API calls
- Gengar still retries once on a 401 after rebuilding the Anthropic client, as a fallback path
...
When an auxiliary task is configured with provider `main`, Gengar resolves that through the same shared runtime path as normal chat. In practice that means:

- env-driven custom endpoints still work
- custom endpoints saved via `gengar model` / `config.yaml` also work
- auxiliary routing can tell the difference between a real saved custom endpoint and the OpenRouter fallback

## Fallback models

Gengar supports a configured fallback model/provider pair, allowing runtime failover when the primary model encounters errors.
