...
sidebar_position: 1
title: "Architecture"
description: "Gengar internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Gengar internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
├── cli.py                    # GengarCLI — interactive terminal UI (~11,500 lines)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── hermes_state.py           # SQLite session/state database with FTS5
├── hermes_constants.py       # GENGAR_HOME, profile-aware paths
...
│   ├── main.py               # Entry point — all `gengar` subcommands (~10,400 lines)
...
│   ├── skills_config.py      # gengar skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # gengar tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # gengar gateway start/stop
│
...
```text
User input → GengarCLI.process_input()
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .gengar.md), tool-use guidance, and model-specific instructions
...
Three discovery sources: `~/.gengar/plugins/` (user), `.gengar/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `gengar plugins` or `config.yaml`.

→ [Plugin Guide](/docs/guides/build-a-gengar-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Gengar as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`gengar -p <name>`) gets its own GENGAR_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
