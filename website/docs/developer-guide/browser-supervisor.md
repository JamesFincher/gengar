...
[PR #12550](https://github.com/jamesfincher/gengar/pull/12550) proposed a
...
(`gengar-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per Gengar `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `gengar-acp`, `gengar-api-server`, core toolsets (gated on CDP reachability)
