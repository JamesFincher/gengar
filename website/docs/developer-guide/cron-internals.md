...
description: "How Gengar stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `hermes_cli/cron.py` | CLI `gengar cron` subcommands |
...
Jobs are stored in `~/.gengar/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `gengar cron` commands are run or during active CLI sessions.
...
```python
# ~/.gengar/scripts/check_competitors.py
...
2. **Environment variable** — `GENGAR_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.gengar/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `gengar cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `gengar cron` CLI provides direct job management:

```bash
gengar cron list                    # Show all jobs
gengar cron create                  # Interactive job creation (alias: add)
gengar cron edit <job_id>           # Edit job configuration
gengar cron pause <job_id>          # Pause a running job
gengar cron resume <job_id>         # Resume a paused job
gengar cron run <job_id>            # Trigger immediate execution
gengar cron remove <job_id>         # Delete a job
```
