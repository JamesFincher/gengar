...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/jamesfincher/gengar/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    hermes_meta = (fm.get("metadata") or {}).get("gengar") or {}
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `gengar skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Gengar"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Gengar ships with a large built-in skill library copied into `~/.gengar/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
