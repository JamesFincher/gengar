...
as ``kind: backend``) or ``~/.gengar/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``gengar tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``gengar tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``gengar tools`` picker.
...
def _images_cache_dir() -> Path:
    """Return ``$GENGAR_HOME/cache/images/``, creating parents as needed."""
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$GENGAR_HOME/cache/images/``.
