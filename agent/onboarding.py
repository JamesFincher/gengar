...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Gengar starts and finds ``~/.openclaw/``.

    Points users at ``gengar claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``gengar claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Gengar, run "
        "`gengar claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `gengar claw cleanup` (renames it to ~/.openclaw.pre-migration — "
