"""Anthropic Messages API adapter for Gengar.

Translates between Gengar's internal OpenAI-style message format and
...
# Gengar effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Gengar historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Gengar)
    ...
    # 1. Gengar-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Gengar configs.
...
# ── Gengar-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.gengar/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_GENGAR_OAUTH_FILE = get_hermes_home() / ".anthropic_oauth.json"
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Gengar-native OAuth PKCE flow and return credential state."""
    import time
    import webbrowser
    ...
    print()
    print("Authorize Gengar with your Claude Pro/Max subscription.")
    print()
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Gengar-managed OAuth credentials from ~/.gengar/.anthropic_oauth.json."""
    if _GENGAR_OAUTH_FILE.exists():
        try:
            data = json.loads(_GENGAR_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Gengar OAuth credentials: %s", e)
    return None
    ...
    *base_url* is a Kimi / Moonshot host), unsigned thinking blocks
    ...
    for m in messages:
        ...
        if role == "assistant":
            ...
            # can validate the message history.  See gengar#13848.
            #
    ...
    # are preserved.  See gengar#13848 (Kimi) and #16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Gengar", "Claude Code")
                text = text.replace("Gengar agent", "Claude Code")
                text = text.replace("gengar", "claude-code")
                text = text.replace("James Fincher", "Anthropic")
                block["text"] = text
    ...
    # silently hides reasoning text that Gengar surfaces in its CLI. We
