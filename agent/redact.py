...
# `export GENGAR_REDACT_SECRETS=true`) cannot enable/disable redaction
# mid-session.  OFF by default — user must opt in via
# `security.redact_secrets: true` in config.yaml (bridged to this env var
# in hermes_cli/main.py and gateway/run.py) or `GENGAR_REDACT_SECRETS=true`
# in ~/.gengar/.env.
_REDACT_ENABLED = os.getenv("GENGAR_REDACT_SECRETS", "").lower() in ("1", "true", "yes", "on")
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across Gengar — used by
    ``gengar config``, ``gengar status``, ``gengar dump``, and anywhere
