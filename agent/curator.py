...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.gengar/config.yaml. Tolerates missing file."""
    try:
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``gengar update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``gengar curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `gengar curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Gengar' background skill CURATOR. This is an "
    ...
    "into ~/.gengar/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: gengar-config-*, gengar-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.gengar/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.gengar/logs/curator/``)
    ...
    authored skill data in ``~/.gengar/skills/``.
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every gengar install already has it
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.gengar/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.gengar/skills/.archive/` for "
            "safety and can be restored via `gengar curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.gengar/skills/.archive/`. "
            "Restore any via `gengar curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `gengar curator restore <name>`")
    lines.append("- All archives live under `~/.gengar/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `gengar curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `gengar curator status` can point at it.
        try:
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``gengar model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `gengar model` → auxiliary
