...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.gengar/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Gengar ↔ models.dev
# ---------------------------------------------------------------------------

# Gengar provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Gengar (built lazily)
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Gengar provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal Gengar agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a Gengar provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Gengar ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Gengar or models.dev provider ID.  Tries exact match then
