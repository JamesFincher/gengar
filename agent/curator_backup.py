...
A pre-run snapshot of ``~/.gengar/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.gengar/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.gengar/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.gengar/cron/jobs.json``)."""
...
def snapshot_skills(reason: str = "manual") -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.gengar/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.gengar/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.gengar/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.gengar/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `gengar curator rollback --list` to see available snapshots)",
            None,
        )
