...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.gengar/auth.json active provider)
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://gengar.jamesfincher.com",
    "X-OpenRouter-Title": "Gengar",
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``GENGAR_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``GENGAR_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("GENGAR_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("GENGAR_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
NOUS_EXTRA_BODY = {"tags": ["product=gengar"]}
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Gengar)",
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.gengar/auth.json for an active Nous provider.
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Gengar auth store.
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `gengar setup` or `gengar model` and enter a valid http(s) base URL."
        ) from exc
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    scenario where a user switches providers via `gengar model` but the
    #    old OPENAI_BASE_URL lingers in ~/.gengar/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: gengar model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.gengar/.env",
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: gengar auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: gengar model)")
                return None, None
...
def _build_call_kwargs(
    ...
    if provider == "nous" or auxiliary_is_nous:
        merged_extra.setdefault("tags", []).extend(["product=gengar"])
    if merged_extra:
