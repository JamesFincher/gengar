...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``GENGAR_PLATFORM`` env var and
    ``GENGAR_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = (
            os.getenv("GENGAR_PLATFORM")
            or get_session_env("GENGAR_SESSION_PLATFORM")
        )
    except Exception:
        resolved_platform = os.getenv("GENGAR_PLATFORM")
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.gengar.config``
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.gengar/skills/ and return a mapping of /command -> skill info.
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.gengar/skills/`` and any ``skills.external_dirs`` so the
